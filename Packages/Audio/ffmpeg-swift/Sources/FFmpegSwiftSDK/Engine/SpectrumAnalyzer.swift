...
public final class SpectrumAnalyzer {
    ...
    private struct VisualDelivery {
        let rawSpectrum: ((_ magnitudes: [Float], _ sampleRate: Double, _ rms: Float) -> Void)?
        let spectrum: SpectrumCallback?
        let smoothing: Float
    }
    ...
    /// Visual FFT never needs to run faster than its renderer. A zero interval
    /// preserves the historical unrestricted behavior for existing clients.
    private var storedVisualMinimumInterval: TimeInterval = 0
    private var lastVisualDeliveryUptime: TimeInterval = 0
    ...
    /// Minimum cadence for visual callbacks. The gate is applied before ring
    /// buffer linearization and FFT work, so high sample-rate sources do not
    /// analyze multiple windows for a renderer that can only present 20–30 fps.
    public var visualMinimumInterval: TimeInterval {
        get { configurationLock.monoWithLock { storedVisualMinimumInterval } }
        set {
            configurationLock.monoWithLock {
                let interval = max(0, newValue)
                guard abs(interval - storedVisualMinimumInterval) > 0.000_1 else { return }
                storedVisualMinimumInterval = interval
                lastVisualDeliveryUptime = 0
            }
        }
    }
    ...
    public func removeAnalysisObserver(_ token: UUID) {
        _ = configurationLock.monoWithLock {
    ...
    func feed(samples: UnsafePointer<Float>, frameCount: Int, channelCount: Int, sampleRate: Double = 44100) {
        ...
        let visualDue = visualActive
            && (lastVisualDeliveryUptime == 0
                || uptime - lastVisualDeliveryUptime >= storedVisualMinimumInterval)
        let analysisDue = analysisObservers.values.contains { entry in
            entry.lastDeliveryUptime == 0
                || uptime - entry.lastDeliveryUptime >= entry.minimumInterval
        }
        let pcmDue = pcmAnalysisObservers.values.contains { entry in
            entry.lastDeliveryUptime == 0
                || uptime - entry.lastDeliveryUptime >= entry.minimumInterval
        }
        let calibrationActive = storedCalibrationIsEnabled
            && storedOnCalibrationSpectrum != nil
        let calibrationDue = calibrationActive
            && (lastCalibrationDeliveryUptime == 0
                || uptime - lastCalibrationDeliveryUptime >= calibrationMinimumInterval)
        let enabled = visualActive
            || !analysisObservers.isEmpty
            || !pcmAnalysisObservers.isEmpty
            || calibrationActive
        let deliveryDue = visualDue || analysisDue || pcmDue || calibrationDue
        configurationLock.unlock()
        ...
        if samplesCollected >= fftSize {
            // Keep the ring current while waiting, but do not enqueue analysis
            // work. Saturating the count lets the first audio block after the
            // deadline deliver the newest complete window immediately.
            guard deliveryDue else {
    ...
    private func consumePendingWindow() {
        ...
        // full analysis pass. Visual callbacks follow their configured cadence.
    ...
    private func takeDueVisualDelivery(at uptime: TimeInterval) -> VisualDelivery {
        configurationLock.monoWithLock {
            let active = storedIsEnabled
                && (storedOnSpectrum != nil || storedOnRawSpectrum != nil)
            let due = active
                && (lastVisualDeliveryUptime == 0
                    || uptime - lastVisualDeliveryUptime >= storedVisualMinimumInterval)
            guard due else {
                return VisualDelivery(rawSpectrum: nil, spectrum: nil, smoothing: storedSmoothing)
            }
            lastVisualDeliveryUptime = uptime
            return VisualDelivery(
                rawSpectrum: storedOnRawSpectrum,
                spectrum: storedOnSpectrum,
                smoothing: storedSmoothing
            )
        }
    }
    ...
    private func performFFT(
        ...
        let visualDelivery = takeDueVisualDelivery(at: deliveryUptime)
        guard visualDelivery.rawSpectrum != nil
                || visualDelivery.spectrum != nil
        ...
        if visualDelivery.rawSpectrum != nil || calibrationCallback != nil || !analysisCallbacks.isEmpty {
        ...
        if let rawCallback = visualDelivery.rawSpectrum {
            let tau: Float = 0.8
            let minDb: Float = -100
            let maxDb: Float = -30
        ...
        if let spectrumCallback = visualDelivery.spectrum {
            let bands = mergeToBands(magnitudes: &scaledMags)
            var smoothed = [Float](repeating: 0, count: bandCount)
            for i in 0..<bandCount {
                smoothed[i] = visualDelivery.smoothing * previousMagnitudes[i]
                    + (1.0 - visualDelivery.smoothing) * bands[i]
            }
