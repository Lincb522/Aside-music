...
function findProtectCode(data, code, purpose) {
  ...
  return protectCodesForPurpose(data, purpose).find(item => item.code === normalizedCode) || null
}
...
app.post('/api/public/register', (req, res) => {
  ...
  // 自签 IPA 与 TestFlight 使用独立保护码池；本路由只接受 IPA 自签保护码。
  normalizeProtectCodeData(data)
  ...
  if (requireProtectCode) {
    if (!protectCode || !protectCode.trim()) {
      return res.status(400).json({ error: '请输入 IPA 自签保护码' })
    }
    matchedCode = findProtectCode(data, protectCode, 'ipa')
    if (!matchedCode) {
      return res.status(403).json({ error: 'IPA 自签保护码无效' })
    }
    if (matchedCode.currentUses >= matchedCode.maxUses) {
      return res.status(403).json({ error: '该 IPA 自签保护码已达使用上限' })
    }
  }
...
app.post('/api/internal/verify-protect-code', (req, res) => {
  ...
  if (!target) {
    return res.status(404).json({ error: requestedPurpose === 'ipa' ? 'IPA 自签保护码不存在' : 'TestFlight 保护码不存在' })
  }
