...
enum GlobalThemeIdGen {
    /// 从 `GlobalThemeId.allCases` 中均匀采样。
    /// 用于 Property 1 / 3 / 4 / 9 / 13 等测试。
    static func any(using rng: inout SeededRNG) -> GlobalThemeId {
    ...
    static func anyExcept(_ exclude: GlobalThemeId, using rng: inout SeededRNG) -> GlobalThemeId {
...
enum HexStringGen {
    ...
    static func sixDigit(using rng: inout SeededRNG, uppercase: Bool = false) -> String {
    ...
    static func variableLength(using rng: inout SeededRNG) -> String {
...
enum AsciiStringGen {
    ...
    static func any(using rng: inout SeededRNG, maxLen: Int = 24) -> String {
    ...
    /// 用于 Property 2 的反例采样。
    static func anyNotMatchingGlobalThemeId(using rng: inout SeededRNG, maxLen: Int = 24) -> String {
...
enum IntGen {
    ...
    static func range(_ lo: Int, _ hi: Int, using rng: inout SeededRNG) -> Int {
    ...
    /// 生成小范围非负整数 [0, hi]。
    static func nonNegative(upTo hi: Int, using rng: inout SeededRNG) -> Int {
...
enum BoolGen {
    ...
    static func any(using rng: inout SeededRNG) -> Bool {
        (rng.next() & 1) == 1
    }

    /// 生成长度为 n 的 [Bool]。用于 Property 12 对"跨帧状态序列"的测试。
    static func sequence(length: Int, using rng: inout SeededRNG) -> [Bool] {
...
enum ColorSchemeGen {
    ...
    static func any(using rng: inout SeededRNG) -> ColorScheme {
        all[Int(rng.next() & 1)]
    }

    /// 返回全量 [.light, .dark] 枚举(供双 scheme 遍历使用)。
    static var allCases: [ColorScheme] { all }
}
