...
struct ClayIconBubble: View {
    ...
    var body: some View {
        RoundedRectangle(cornerRadius: size * 0.42, style: .continuous)
            ...
            .overlay(
                MonoIcon(icon: icon, size: size * 0.43, color: ClayStyle.ink, lineWidth: 1.75)
            )
...
struct ClayPill: View {
    ...
    var body: some View {
        HStack(spacing: icon == nil ? 0 : 6) {
            if let icon {
                MonoIcon(icon: icon, size: 12, color: foreground, lineWidth: 1.65)
            }
...
extension ClaySectionHeader where Action == EmptyView {
    init(title: String, icon: MonoIcon.IconType, tint: Color = ClayStyle.accent) {
