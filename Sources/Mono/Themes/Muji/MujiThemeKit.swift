            ...
            if let actionTitle, let action {
                Button(action: action) {
                    HStack(spacing: 4) {
                        ...
                        MonoIcon(icon: .chevronRight, size: 8, color: MujiStyle.clay, lineWidth: 1.6)
                    }
...
struct MujiIconBadge: View {
    ...
    var body: some View {
        ...
        Circle()
            ...
            .overlay(
                MonoIcon(icon: icon, size: size * 0.4, color: tint, lineWidth: 1.5)
            )
    }
}
...
struct MujiActionPill: View {
    ...
    var body: some View {
        let _ = settings.globalThemeRevision
        HStack(spacing: 7) {
            MonoIcon(icon: icon, size: 13, color: foreground, lineWidth: 1.5)
            Text(title)
