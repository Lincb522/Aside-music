...
struct MangaHomeView: View {
            ...
            Group {
                if let source = viewModel.userProfile?.avatarUrl,
                   let url = URL(string: source) {
                    ...
                    ZStack {
                        MangaComicPalette.paperWarm
                        MonoIcon(icon: .profileFilled, size: size * 0.34, color: MangaComicPalette.ink, lineWidth: 2.2)
                    }
                }
            }
        ...
        return MangaComicPanel(fill: MangaComicPalette.paper, corner: 18, shadow: 4, innerBorder: true) {
            GeometryReader { proxy in
                ...
                ZStack(alignment: .topLeading) {
                    MangaScreentone(opacity: 0.05, gap: 10, fadeFrom: UnitPoint(x: 0.12, y: 0.15))
                    MangaFocusLines(
                        color: MangaComicPalette.ink,
                        opacity: 0.09,
                        focus: UnitPoint(x: 0.85, y: 0.52),
                        clearRadius: coverSide * 0.52
                    )
                    ...
                    MangaSFXNote(size: 34, rotation: 12)
            ...
            .overlay(alignment: .topTrailing) {
                ZStack {
                    ...
                    if isCurrent {
                        ...
                        MonoIcon(icon: .chart, size: 17, color: MangaComicPalette.ink, lineWidth: 2.2)
                    }
                }
    ...
    private var mangaEmptyLeadCover: some View {
        ZStack {
            ...
            MonoIcon(icon: .musicNote, size: 46, color: MangaComicPalette.whiteInk, lineWidth: 2.4)
        }
        .clipShape(MangaComicPanelShape(corner: 12))
        .overlay(MangaComicPanelShape(corner: 12).stroke(MangaComicPalette.ink, lineWidth: 3))
    }
    ...
    private func mangaLeadPlayButton(_ song: Song) -> some View {
        let isCurrent = playerManager.currentSong?.id == song.id
        let icon: MonoIcon.IconType = isCurrent && playerManager.isPlaying ? .pause : .play
        ...
        return Button {
            ...
            HStack(spacing: 8) {
                MangaComicCutCornerShape(cut: 5)
                    ...
                    .overlay(MangaComicCutCornerShape(cut: 5).stroke(MangaComicPalette.ink, lineWidth: 1))
                    .overlay(MonoIcon(icon: icon, size: 11, color: MangaComicPalette.ink, lineWidth: 2.2))
    ...
    private func mangaFeatureTiles(width: CGFloat) -> some View {
        ...
        return HStack(spacing: 8 * layoutScale) {
            ...
            MangaComicFeatureTile(
                ...
                fill: MangaComicPalette.inkSoft,
                foreground: MangaComicPalette.whiteInk
            ) {
    ...
    private var mangaRecommendationSection: some View {
        ...
        .background {
            ZStack {
                ...
                MangaScreentone(opacity: 0.045, gap: 11, fadeFrom: .topLeading)
                MangaComicPaperTexture(opacity: 0.08)
            }
        }
    }
    ...
    private func mangaPlaylistSection(
        ...
        .background {
            ZStack {
                MangaComicPalette.paper
                MangaScreentone(opacity: 0.04, gap: 12, fadeFrom: .bottomTrailing)
                MangaComicPaperTexture(opacity: 0.08)
            }
        }
    }
...
private struct MangaComicMastheadAction: View {
    ...
    private var foreground: Color {
        fill == MangaComicPalette.ink ? MangaComicPalette.whiteInk : MangaComicPalette.ink
    }

    var body: some View {
        Button(action: action) {
            VStack(spacing: 4) {
                MonoIcon(icon: icon, size: 23 * scale, color: foreground, lineWidth: 2.5 * scale)
                ...
                ZStack(alignment: .leading) {
                    ...
                    if icon == .moon {
                        Image("MangaMeditationBackdrop")
                            ...
                            .overlay(MangaComicPalette.ink.opacity(0.2).clipShape(panelShape))
                    } else {
                        MangaComicSpeedLines(
                            color: fill == MangaComicPalette.paper ? MangaComicPalette.ink : MangaComicPalette.whiteInk,
                            opacity: fill == MangaComicPalette.paper ? 0.06 : 0.12
                        )
                        .clipShape(panelShape)
                    }
        ...
        Button(action: action) {
            MangaComicPanel(fill: MangaComicPalette.violet, corner: 14, shadow: 3, innerBorder: true) {
                ZStack(alignment: .leading) {
                    ...
                    MangaFocusLines(
                        color: MangaComicPalette.whiteInk,
                        opacity: 0.07,
                        focus: UnitPoint(x: 0.82, y: 0.35),
                        clearRadius: 72
                    )
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        MangaComicRibbon(
                            text: String(localized: "new_song_express"),
                            fill: MangaComicPalette.paper,
                            foreground: MangaComicPalette.ink
                        )
                        .scaleEffect(0.76, anchor: .leading)
            ...
            HStack(spacing: 11) {
                if rank <= 3 {
                    Text(String(format: "%02d", rank))
                        ...
                        .background(MangaComicCutCornerShape(cut: 5).fill(MangaComicPalette.ink))
                } else {
                    Text(String(format: "%02d", rank))
                        .font(MangaComicPalette.headlineFont(15))
                        .foregroundStyle(MangaComicPalette.mutedInk)
                ...
                MangaComicRoundControl(icon: isCurrent ? .pause : .play, fill: MangaComicPalette.ink, size: 34)
            }
            .padding(.horizontal, 10)
            .padding(.vertical, 8)
            .background {
                ZStack {
                    MangaComicPanelShape(corner: 11).fill(MangaComicPalette.paperWarm)
                    if isCurrent {
                        MangaScreentone(opacity: 0.09, gap: 7)
                            .clipShape(MangaComicPanelShape(corner: 11))
                    }
                }
            }
            .overlay(MangaComicPanelShape(corner: 11).stroke(MangaComicPalette.ink, lineWidth: isCurrent ? 3 : 2.3))
        }
        .buttonStyle(MangaComicPressButtonStyle())
    }
}
