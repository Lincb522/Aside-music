...
struct PetWhitePageHeader<Accessory: View>: View {
    ...
    init(
        ...
        title: String,
        subtitle: String = "",
        icon: MonoIcon.IconType = .catLife,
...
struct PetWhiteSpinningCoverDisc: View {
    ...
    private var coverPlaceholder: some View {
        Circle()
            ...
            .overlay(
                MonoIcon(icon: .musicNote, size: size * 0.20, color: PetWhiteStyle.inkMuted, lineWidth: 1.6)
            )
    }
}
...
struct PetWhiteChevronIcon: View {
    ...
    private var fallbackChevron: some View {
        MonoIcon(icon: direction.fallbackIcon, size: size, color: fallbackColor)
    }
}
...
struct PetWhiteDisclosureChevron: View {
    ...
    var color: Color = .monoTextSecondary.opacity(0.8)
    ...
    var body: some View {
        if PetWhiteStyle.isActive {
            ...
            MonoIcon(icon: .chevronRight, size: size, color: color, lineWidth: lineWidth)
...
struct PetWhiteSectionTitle: View {
    let title: String
    var detail: String?
    var icon: MonoIcon.IconType = .catLife
    var assetName: String?
    var tint: Color = PetWhiteStyle.mint
    var action: (() -> Void)?
