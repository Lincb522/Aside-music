...
struct AudioLabView: View {
        ...
        ZStack {
            ...
            if showAppliedToast {
                VStack {
                    Spacer()
                    HStack(spacing: 8) {
                        MonoIcon(icon: .checkmark, size: 16, color: .monoAccentForeground)
                        Text("已应用智能音效")
            ...
            HStack(spacing: 14) {
                ZStack {
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .fill(labManager.isSmartEffectsEnabled ? Color.monoAccent.opacity(0.15) : Color.monoSeparator)
                        .frame(width: 44, height: 44)
                    MonoIcon(icon: .sparkle, size: 20, color: labManager.isSmartEffectsEnabled ? .monoAccent : .monoTextSecondary)
                }
        ...
        return Button(action: {
            ...
            VStack(spacing: 4) {
                MonoIcon(
                    ...
                    size: 16,
                    color: isSelected ? .monoAccentForeground : .monoTextSecondary
                )
                Text(mode.rawValue)
                    .font(.rounded(size: 12, weight: .medium))
                Text(mode == .file ? String(localized: "更准确") : String(localized: "更快速"))
                    .font(.rounded(size: 10))
                    .foregroundColor(isSelected ? .monoAccentForeground.opacity(0.8) : .monoTextSecondary.opacity(0.6))
            }
            .foregroundColor(isSelected ? .monoAccentForeground : .monoTextSecondary)
            .frame(maxWidth: .infinity)
            .padding(.vertical, 10)
            .background(
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .fill(isSelected ? Color.monoAccent : Color.monoSeparator)
            )
        }
        .buttonStyle(.plain)
    }
    ...
    private var analyzingCard: some View {
        VStack(spacing: 16) {
            ...
            ZStack {
                Circle()
                    .stroke(Color.monoSeparator, lineWidth: 4)
                    .frame(width: 60, height: 60)
                
                Circle()
                    .trim(from: 0, to: CGFloat(labManager.analysisProgress))
                    .stroke(Color.monoAccent, style: StrokeStyle(lineWidth: 4, lineCap: .round))
                    .frame(width: 60, height: 60)
                    .rotationEffect(.degrees(-90))
                    .animation(.easeInOut(duration: 0.3), value: labManager.analysisProgress)
                
                MonoIcon(icon: .waveform, size: 24, color: .monoAccent)
            }
        ...
        VStack(spacing: 20) {
            ...
            HStack {
                ...
                Button(action: {
                    ...
                    HStack(spacing: 6) {
                        MonoIcon(icon: .refresh, size: 14, color: .monoTextSecondary)
                        Text("重新分析")
                    ...
                    .padding(.vertical, 8)
                    .background(Color.monoSeparator)
                    .clipShape(Capsule())
                }
                .buttonStyle(.plain)
            }
    ...
    private func recommendedTag(_ text: String) -> some View {
        Text(text)
            ...
            .padding(.horizontal, 10)
            .padding(.vertical, 6)
            .background(Color.monoAccent.opacity(0.1))
            .clipShape(Capsule())
    }
    ...
    private func smartEQCurveView(gains: [Float]) -> some View {
        GeometryReader { geo in
            ...
            ZStack {
                ...
                ForEach([0.25, 0.5, 0.75], id: \.self) { ratio in
                    ...
                    .stroke(Color.monoSeparator.opacity(0.3), lineWidth: 0.5)
                }
                ...
                .stroke(Color.monoTextSecondary.opacity(0.3), lineWidth: 1)
                ...
                if points.count >= 2 {
                    ...
                    .stroke(Color.monoAccent, lineWidth: 2)
        ...
        .background(
            RoundedRectangle(cornerRadius: 8, style: .continuous)
                .fill(Color.monoSeparator.opacity(0.2))
        )
    }
    ...
    private var noAnalysisCard: some View {
        VStack(spacing: 16) {
            MonoIcon(icon: .waveform, size: 40, color: .monoTextSecondary.opacity(0.5))
            ...
            if PlayerManager.shared.currentSong != nil {
                Button(action: {
                    ...
                    HStack(spacing: 8) {
                        MonoIcon(icon: .sparkle, size: 16, color: .monoAccent)
                        Text("立即分析")
                            .font(.rounded(size: 14, weight: .medium))
                    }
                    .foregroundColor(.monoAccent)
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack(alignment: .top, spacing: 8) {
                    MonoIcon(icon: .download, size: 14, color: .monoAccent)
                ...
                HStack(alignment: .top, spacing: 8) {
                    MonoIcon(icon: .waveform, size: 14, color: .monoAccent)
                    VStack(alignment: .leading, spacing: 2) {
                        Text("实时分析")
                            .font(.rounded(size: 13, weight: .medium))
                            .foregroundColor(.monoTextPrimary)
    ...
    private func featureRow(icon: MonoIcon.IconType, title: String, description: String) -> some View {
        HStack(spacing: 12) {
            ZStack {
                RoundedRectangle(cornerRadius: 8, style: .continuous)
                    .fill(Color.monoSeparator)
                    .frame(width: 36, height: 36)
                MonoIcon(icon: icon, size: 16, color: .monoTextSecondary)
            }
