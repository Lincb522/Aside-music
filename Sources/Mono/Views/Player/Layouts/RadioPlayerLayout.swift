...
struct RadioPlayerLayout: View {
    ...
    private var transportStrip: some View {
        HStack(spacing: 14) {
            skeuBtn(icon: player.mode.monoIcon, size: 36) {
            ...
            Button {
                ...
                ZStack {
                    if player.isLoading {
                        ...
                        MonoIcon(icon: player.isPlaying ? .pause : .play, size: 22, color: btnFg)
    ...
    private func skeuBtn(icon: MonoIcon.IconType?, size: CGFloat, label: String? = nil, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Group {
                if let icon {
                    MonoIcon(icon: icon, size: size * 0.40, color: btnFg.opacity(0.8))
                } else if let label {
                    Text(label)
