    ...
    func mangaBackground(size: CGSize) -> some View {
        ZStack {
            ...
            Canvas { context, canvasSize in
                ...
                while y < canvasSize.height + gap {
                    var x: CGFloat = shifted ? gap : gap / 2
                    while x < canvasSize.width + gap {
                        let rect = CGRect(
                            x: x - dotRadius,
                            y: y - dotRadius,
                            width: dotRadius * 2,
    ...
    var topBar: some View {
        HStack {
            Button(action: { dismiss() }) {
                MonoSymbolIcon(name: "chevron.down", size: 16, color: ink)
            ...
            HStack(spacing: 3) {
                MonoIcon(icon: .comment, size: 10, color: ink, lineWidth: 1.8)
                Text("CHAT")
            ...
            Button(action: { showMoreMenu.toggle() }) {
                MonoIcon(icon: .more, size: 16, color: ink, lineWidth: 1.8)
                    .frame(width: 34, height: 34)
                    .background(
                        RoundedRectangle(cornerRadius: 12, style: .continuous)
                            .fill(bubbleWhite)
        ...
        Group {
            if let url = player.currentSong?.coverUrl?.sized(200) {
                ...
                ZStack {
                    ...
                    MonoIcon(icon: .musicNote, size: size * 0.35, color: inkSub.opacity(0.5), lineWidth: 1.8)
                }
            }
        }
        ...
        Group {
            if lyricVM.hasLyrics && !lyricVM.lyrics.isEmpty {
                                ...
                                if !line.text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                                    ...
                                    if index <= lyricVM.currentLineIndex {
                                        chatBubble(
                                            ...
                                            translation: line.translation?.monoLyricDisplayText,
                                            isLeft: isLeft,
                                            isCurrent: isCurrent,
                                            index: index
                                        )
                ...
                VStack(spacing: 16) {
                    ...
                    VStack(spacing: 12) {
                        MonoIcon(icon: .comment, size: 50, color: inkSub.opacity(0.25), lineWidth: 1.8)
        ...
        Group {
            if isLeft {
                ...
                if let url = player.currentSong?.coverUrl?.sized(100) {
                    ...
                    ZStack {
                        labelYellow.opacity(0.4)
                        MonoIcon(icon: .microphone, size: 13, color: inkSub, lineWidth: 1.7)
                    }
                }
            } else {
                if let avatarStr = userAvatarUrl, let url = URL(string: avatarStr) {
                    ...
                    ZStack {
                        decoBlue.opacity(0.5)
                        MonoIcon(icon: .profileFilled, size: 17, color: ink.opacity(0.5), lineWidth: 1.7)
                    }
                }
            }
        }
...
extension MangaChatPlayerLayout {
        ...
        VStack(spacing: 12) {
            ...
            HStack(spacing: 0) {
                ...
                Button(action: { player.switchMode() }) {
                    MonoIcon(icon: player.mode.monoIcon, size: 16, color: inkSub)
                ...
                if player.currentSong != nil {
                    if AppConfig.Features.downloadEnabled, let song = player.currentSong {
                        ...
                        Button {
                            ...
                            MonoIcon(icon: .immersive, size: 16, color: inkSub)
                                .frame(width: 36, height: 36)
                        }
                        .buttonStyle(MonoBouncingButtonStyle())
                    }
    ...
    func mangaButton(symbolName: String, w: CGFloat, h: CGFloat, isPlay: Bool) -> some View {
        MonoSymbolIcon(name: symbolName, size: min(w, h) * 0.44, color: isPlay ? .white : ink)
    ...
    func mangaControlIcon(symbolName: String, size: CGFloat, dimmed: Bool = false) -> some View {
        MonoSymbolIcon(name: symbolName, size: size, color: dimmed ? inkSub.opacity(0.3) : inkSub)
    }
