...
struct MotoPagerLayout: View {
    ...
    private func printNewLyric(index: Int) {
        ...
        fullLyricText = line.text.monoLyricDisplayText
        typingText = ""
        isTyping = true
        typingDone = false
        ...
        VStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    HStack(alignment: .top, spacing: 0) {
                        if typingText.isEmpty && fullLyricText.isEmpty {
                            ...
                            Text(
                                typingText.monoLyricDisplayText
            ...
            HStack {
                ...
                HStack(spacing: 5) {
                    ...
                    // 爱心
                    if let song = player.currentSong {
                        LikeButton(songId: song.id, isQQMusic: song.isQQMusic, song: song, size: 14, activeColor: accentColor, inactiveColor: brandSubColor)
                            .frame(width: 25, height: 25)
                    ...
                    Button(action: { player.previous() }) {
                        MonoIcon(icon: .previous, size: 11, color: brandSubColor, lineWidth: 1.5)
                            .frame(width: 25, height: 25)
                            .background(smallBtnColor)
                            .clipShape(Circle())
                            .shadow(color: .black.opacity(0.1), radius: 2, y: 1)
                    ...
                    Button(action: { player.next() }) {
                        MonoIcon(icon: .next, size: 11, color: brandSubColor, lineWidth: 1.5)
                            .frame(width: 25, height: 25)
                            .background(smallBtnColor)
                            .clipShape(Circle())
                            .shadow(color: .black.opacity(0.1), radius: 2, y: 1)
    ...
    private var paperView: some View {
        ...
        VStack(spacing: 0) {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(isEjecting ? pendingEjectText.monoLyricDisplayText : " ")
                    .font(
                        MonoPlayerFont.activeFont(
                            size: 16,
                            weight: .bold,
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                // Content
                Text(content.monoLyricDisplayText)
                    .font(
                        MonoPlayerFont.activeFont(
                            size: 16,
                            weight: .bold,
        ...
        VStack {
            HStack {
                Button(action: { dismiss() }) {
                    MonoSymbolIcon(name: "chevron.down", size: 23, color: topBtnFgColor)
