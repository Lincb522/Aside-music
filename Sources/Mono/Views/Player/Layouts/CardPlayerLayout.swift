    ...
    var topBar: some View {
        HStack {
            Button(action: { dismiss() }) {
                MonoSymbolIcon(name: "chevron.down", size: 23, color: .primary)
            ...
            Button(action: { showMoreMenu.toggle() }) {
                MonoIcon(icon: .more, size: 23, color: .primary, lineWidth: 1.5)
                    .frame(width: 44, height: 44)
    ...
    func visualCard(geo: GeometryProxy) -> some View {
        ...
        ZStack {
            ...
            if abs(dragOffset.width) > 50 {
                MonoSymbolIcon(name: dragOffset.width > 0 ? "backward.fill" : "forward.fill", size: 40, color: .white)
                    .padding()
        ...
        VStack(spacing: 16) {
            HStack(spacing: 0) {
                Button(action: { player.switchMode() }) {
                    MonoIcon(icon: player.mode.monoIcon, size: 20, color: .secondary)
                ...
                Button(action: { player.previous() }) {
                    MonoIcon(icon: .previous, size: 24, color: .primary)
                ...
                Button(action: { player.togglePlayPause() }) {
                    ZStack {
                        ...
                        MonoIcon(icon: player.isPlaying ? .pause : .play, size: 24, color: .white)
                ...
                Button(action: { player.next() }) {
                    MonoIcon(icon: .next, size: 24, color: .primary)
                        .frame(width: 60, height: 60)
                        .contentShape(Rectangle())
                }
                .buttonStyle(MonoBouncingButtonStyle())
                ...
                Button(action: { showPlaylist = true }) {
                    MonoIcon(icon: .list, size: 20, color: .secondary)
                        .frame(width: 50, height: 60)
                        .contentShape(Rectangle())
                }
                .buttonStyle(MonoBouncingButtonStyle())
            ...
            if let song = player.currentSong {
                HStack(spacing: 0) {
                    Button { showComments = true } label: {
                        MonoIcon(icon: .comment, size: 22, color: .secondary, lineWidth: 1.4)
                            .frame(width: 44, height: 44)
                            .contentShape(Rectangle())
                    }
                    .buttonStyle(MonoBouncingButtonStyle())
