    ...
    private var topBar: some View {
        HStack {
            Button(action: { dismiss() }) {
                MonoIcon(icon: .close, size: 22, color: textPrimary)
            ...
            Button(action: {
                ...
                MonoIcon(icon: .more, size: 22, color: textPrimary)
                    .frame(width: 44, height: 44)
                    .background(shellColor)
                    .clipShape(Circle())
                    .overlay(Circle().stroke(strokeColor, lineWidth: 1))
    ...
    private var controls: some View {
        HStack(spacing: 0) {
            ...
            Button(action: { player.switchMode() }) {
                MonoIcon(icon: player.mode.monoIcon, size: 18, color: textMuted)
                    .frame(width: 44, height: 44)
            ...
            Button(action: { player.previous() }) {
                MonoIcon(icon: .previous, size: 22, color: textPrimary)
            ...
            Button(action: { player.next() }) {
                MonoIcon(icon: .next, size: 22, color: textPrimary)
                    .frame(width: 48, height: 48)
                    .contentShape(Rectangle())
            }
            .buttonStyle(MonoBouncingButtonStyle())
            ...
            Button(action: { showPlaylist = true }) {
                MonoIcon(icon: .list, size: 18, color: textMuted)
                    .frame(width: 44, height: 44)
                    .contentShape(Rectangle())
            }
            .buttonStyle(MonoBouncingButtonStyle())
