...
private struct TypewriterLyricsView: View {
        ...
        ScrollViewReader { proxy in
            ...
            .onAppear {
                if vm.currentLineIndex > 0 {
                    proxy.monoRestoreLyricPosition(anchor: .top) { vm.currentLineIndex }
                }
            }
        }
    ...
    private func currentLine(_ line: LyricLine, realTime: TimeInterval) -> some View {
        let displayText = line.text.monoLyricDisplayText
...
struct TypewriterPlayerLayout: View {
    ...
    var body: some View {
        ...
        .monoSheet(isPresented: $showPlaylist, preset: .standard) { PlaylistPopupView() }
    ...
    private func roundKey(icon: MonoIcon.IconType, size: CGFloat, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            ZStack {
                roundKeycap(size: size)
                MonoIcon(icon: icon, size: size * 0.4, color: keyText)
            }
            .contentShape(Circle())
        }
    ...
    private func labelKey(text: String, icon: MonoIcon.IconType, disabled: Bool = false, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            HStack(spacing: 6) {
                MonoIcon(icon: icon, size: 13, color: disabled ? keyText.opacity(0.3) : keyText)
                Text(text)
                    .font(.system(size: 10, weight: .bold, design: .monospaced))
    ...
    private var topBar: some View {
        HStack {
            Button { dismiss() } label: {
                MonoIcon(icon: .close, size: 20, color: topBtnFg)
                    .frame(width: 44, height: 44)
                    .monoGlassCircle()
                    .contentShape(Circle())
            }
            ...
            Button {
                ...
                MonoIcon(icon: .more, size: 20, color: topBtnFg)
                    .frame(width: 44, height: 44)
                    .monoGlassCircle()
                    .contentShape(Circle())
            }
