                    ...
                    ZStack {
                        if showLyrics {
                            ...
                            ZStack {
                                // 唱片柔和阴影（不跟随旋转）
                                Circle()
                                    .fill(Color.black.opacity(colorScheme == .dark ? 0.5 : 0.08))
    ...
    var headerBar: some View {
        HStack {
            Button(action: { dismiss() }) {
                MonoIcon(icon: .chevronRight, size: 22, color: contentColor.opacity(0.5))
            ...
            Button(action: {
                ...
                MonoIcon(icon: .more, size: 22, color: contentColor.opacity(0.5))
                    .frame(width: 44, height: 44)
                    .contentShape(Circle())
            }
            .buttonStyle(MonoBouncingButtonStyle())
    ...
    var songInfoSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 10) {
                ...
                if let song = player.currentSong {
                    LikeButton(songId: song.id, isQQMusic: song.isQQMusic, song: song, size: 20, activeColor: .red, inactiveColor: secondaryColor)
                }
            }
        }
        ...
        VStack(spacing: 16) {
            HStack(spacing: 12) {
                ...
                Button(action: { player.togglePlayPause() }) {
                    HStack(spacing: 8) {
                        if player.isLoading {
                            ProgressView()
                                .progressViewStyle(CircularProgressViewStyle(tint: .monoIconForeground))
                                .scaleEffect(0.8)
                        } else {
                            MonoIcon(
                ...
                Button(action: { player.previous() }) {
                    MonoIcon(icon: .previous, size: 22, color: contentColor)
                ...
                Button(action: { player.next() }) {
                    MonoIcon(icon: .next, size: 22, color: contentColor)
                        .frame(width: 50, height: 50)
                        .monoGlass(cornerRadius: 16)
                }
                .buttonStyle(MonoBouncingButtonStyle())
            ...
            HStack(spacing: 0) {
                ...
                Button { showPlaylist = true } label: {
                    MonoIcon(icon: .list, size: 23, color: secondaryColor, lineWidth: 1.45)
                    ...
                    if AppConfig.Features.downloadEnabled {
                        ...
                        Button {
                            ...
                            MonoIcon(
                                icon: .playerDownload,
                                size: 22,
                                color: downloadStatus.isDownloaded(song: song) ? secondaryColor : secondaryColor,
                                lineWidth: 1.4
                            )
                            .frame(width: 44, height: 44)
                            .contentShape(Rectangle())
