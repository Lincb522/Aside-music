...
extension PixelPlayerLayout {
    ...
    private func pixelButton(icon: MonoIcon.IconType, rotation: Double = 0) -> some View {
        Color.clear
            .frame(width: 40, height: 40)
            .monoGlass(cornerRadius: 0)
            .overlay(Rectangle().stroke(fg.opacity(0.5), lineWidth: 2))
            .overlay(
                MonoIcon(icon: icon, size: 20, color: fg)
...
extension PixelPlayerLayout {
        ...
        VStack(spacing: 10) {
            ...
            HStack(spacing: 20) {
                Button(action: { player.previous() }) {
                    pixelControlButton {
                        MonoIcon(icon: .previous, size: 20, color: fg)
                    }
                }
                .buttonStyle(MonoBouncingButtonStyle())
                ...
                Button(action: { player.next() }) {
                    pixelControlButton {
                        MonoIcon(icon: .next, size: 20, color: fg)
                    }
                }
                .buttonStyle(MonoBouncingButtonStyle())
            }
            ...
            // 功能行
            HStack(spacing: 0) {
                funcButton(icon: player.mode.monoIcon) { player.switchMode() }
                Spacer()
                if let song = player.currentSong {
    ...
    private func funcButton(icon: MonoIcon.IconType, tint: Color? = nil, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            ZStack {
                Rectangle()
                    ...
                    .frame(width: 40, height: 36)
                    .offset(x: 1, y: 1)
                MonoIcon(icon: icon, size: 16, color: tint ?? fg, lineWidth: 1.5)
                    .frame(width: 40, height: 36)
                    .background(fg.opacity(0.08))
