...
extension ImmersiveLyricPlayerLayout {
    ...
    var transportRow: some View {
        HStack {
            ...
            Button(action: { player.previous() }) {
                MonoIcon(icon: .previous, size: 30, color: .white)
            }
            .buttonStyle(MonoBouncingButtonStyle())
            ...
            Button(action: { player.togglePlayPause() }) {
                if player.isLoading {
                    ...
                    MonoIcon(icon: player.isPlaying ? .pause : .play, size: 48, color: .white)
                }
            }
            .buttonStyle(MonoBouncingButtonStyle())
            ...
            Button(action: { player.next() }) {
                MonoIcon(icon: .next, size: 30, color: .white)
            }
            .buttonStyle(MonoBouncingButtonStyle())
    ...
    @ViewBuilder
    func utilityButton(icon: MonoIcon.IconType, active: Bool = false, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            MonoIcon(icon: icon, size: 19, color: active ? .black : .white.opacity(0.78))
    ...
    var lyricTopBar: some View {
        HStack(spacing: 12) {
            ...
            Button(action: { toggleLyrics() }) {
                MonoIcon(icon: .chevronDown, size: 16, color: .white.opacity(0.9))
                    .frame(width: 32, height: 32)
                    .background(Circle().fill(Color.white.opacity(0.14)))
            }
            .buttonStyle(MonoBouncingButtonStyle())
