    ...
    private func modeTile(cell: CGFloat) -> some View {
        Button(action: { player.switchMode() }) {
            gameTile(value: 2, cell: cell) {
                MonoIcon(icon: player.mode.monoIcon, size: 14, color: tileFg(2))
            }
        }.buttonStyle(MonoBouncingButtonStyle())
    }
    ...
    private func likeTile(cell: CGFloat) -> some View {
        ...
        return Button {
            ...
            gameTile(value: 4, cell: cell) {
                MonoIcon(icon: isLiked ? .liked : .like, size: 17, color: isLiked ? .red : tileFg(4), lineWidth: 1.8)
    ...
    private func prevTile(cell: CGFloat) -> some View {
        Button(action: { player.previous() }) {
            gameTile(value: 8, cell: cell) {
                MonoIcon(icon: .previous, size: 17, color: tileFg(8), lineWidth: 1.8)
            }
        }.buttonStyle(MonoBouncingButtonStyle())
    }
    ...
    private func nextTile(cell: CGFloat) -> some View {
        Button(action: { player.next() }) {
            gameTile(value: 16, cell: cell) {
                MonoIcon(icon: .next, size: 17, color: tileFg(16), lineWidth: 1.8)
            }
        }.buttonStyle(MonoBouncingButtonStyle())
    }
    ...
    private func qualityTile(cell: CGFloat) -> some View {
        Button { showQualitySheet = true } label: {
            ZStack(alignment: .topLeading) {
                ...
                VStack(spacing: 2) {
                    MonoIcon(icon: .waveform, size: 15, color: tileFg(64), lineWidth: 1.8)
...
extension Game2048PlayerLayout {
    private var bottomBar: some View {
        HStack(spacing: 10) {
            ...
            Spacer()
            HStack(spacing: 3) {
                MonoSymbolIcon(name: "hand.draw", size: 11, color: headerText.opacity(0.25))
    ...
    private func bottomBtn(icon: String, label: String, dim: Bool = false,
        ...
        Button(action: action) {
            VStack(spacing: 2) {
                MonoSymbolIcon(name: icon, size: 14, color: dim ? headerText.opacity(0.2) : headerText.opacity(0.6))
            ...
            VStack(spacing: 0) {
                HStack {
                    ...
                    Button {
                        ...
                        MonoIcon(icon: .close, size: 14, color: .white.opacity(0.6), lineWidth: 1.6)
