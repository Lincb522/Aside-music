...
struct AriaShelfWall: View {
    ...
    private var searchTopBar: some View {
        HStack(spacing: 10) {
            Button {
                ...
                MonoIcon(icon: .close, size: 14, color: .white.opacity(0.9), lineWidth: 1.8)
    ...
    private func searchOverlay(geo: GeometryProxy) -> some View {
        VStack(spacing: 10) {
            ...
            if trimmedQuery.isEmpty {
                VStack(spacing: 7) {
                    MonoIcon(icon: .search, size: 26, color: .white.opacity(0.16), lineWidth: 2)
    ...
    private var searchField: some View {
        HStack(spacing: 8) {
            MonoIcon(icon: .search, size: 14, color: .white.opacity(0.4), lineWidth: 1.7)
    ...
    private func card(song: Song, index: Int, size: CGFloat, pitch: CGFloat) -> some View {
        ...
        CachedAsyncImage(url: song.coverUrl?.sized(500)) {
            Rectangle().fill(Color.white.opacity(0.06))
                .overlay(
                    MonoIcon(icon: .album, size: 44, color: .white.opacity(0.15))
                )
        }
    ...
    private var closeButton: some View {
        Button {
            ...
            MonoIcon(icon: .close, size: 16, color: .white.opacity(0.9))
                .frame(width: 44, height: 44)
                .background(
                    Circle()
                        .fill(.ultraThinMaterial)
