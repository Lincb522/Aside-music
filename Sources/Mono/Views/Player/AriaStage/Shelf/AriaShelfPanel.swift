...
struct AriaUnifiedPanel: View {
    ...
    private var coverBlock: some View {
        ...
        return CachedAsyncImage(url: player.currentSong?.coverUrl?.sized(500)) {
            Rectangle().fill(Color.white.opacity(0.06))
                .overlay(
                    MonoIcon(icon: .album, size: 36, color: .white.opacity(0.2))
                )
        }
    ...
    private func tabButton(_ target: AriaPanelTab, icon: MonoIcon.IconType) -> some View {
        ...
        return Button {
            ...
            MonoIcon(icon: icon, size: 16, color: .white)
        ...
        VStack(spacing: 12) {
            ...
            HStack(spacing: 22) {
                ...
                Button {
                    ...
                    MonoIcon(icon: player.mode.monoIcon, size: 20, color: .white.opacity(0.6))
    ...
    private var shelfPlatformSwitcher: some View {
        HStack(spacing: 5) {
            ForEach([MusicSource.netease, .qqmusic, .qishui, .appleMusic], id: \.self) { source in
    ...
    private var shelfSearchField: some View {
        HStack(spacing: 8) {
            MonoIcon(icon: .search, size: 13, color: .white.opacity(0.4), lineWidth: 1.7)
            ...
            if isSearching {
                ...
                Button {
                    ...
                    MonoIcon(icon: .xmark, size: 9, color: .white.opacity(0.45), lineWidth: 1.7)
                ...
                Button {
                    ...
                    MonoIcon(icon: .chevronDown, size: 11, color: .white.opacity(0.45), lineWidth: 1.7)
                        .frame(width: 22, height: 22)
                        .background(Circle().fill(Color.white.opacity(0.08)))
                }
                .buttonStyle(.plain)
    ...
    private func performShelfSearch(_ keyword: String) async {
        ...
        do {
            ...
            case .kugou:
                for try await result in APIService.shared
                    .searchKugouSongsWithTotal(keyword: keyword).values {
                    guard !Task.isCancelled else { return }
