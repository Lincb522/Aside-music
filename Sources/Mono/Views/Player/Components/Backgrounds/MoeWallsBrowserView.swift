...
struct MoeWallsBrowserView: View {
    ...
    private var header: some View {
        HStack(spacing: 12) {
            Button { dismiss() } label: {
                MonoIcon(icon: .back, size: 18, color: .white)
            ...
            Button {
                ...
                MonoIcon(icon: .refresh, size: 17, color: .white.opacity(0.88))
                    .frame(width: 44, height: 44)
                    .contentShape(Rectangle())
            }
            .buttonStyle(.plain)
    ...
    private var searchBar: some View {
        HStack(spacing: 9) {
            MonoIcon(icon: .search, size: 16, color: .white.opacity(0.48))
            ...
            Button {
                ...
                MonoIcon(icon: .close, size: 12, color: .white.opacity(0.72))
            ...
            Button { commitSearch() } label: {
                Group {
                    if model.isSearching {
                        ...
                        MonoIcon(icon: .search, size: 15, color: .white)
                    }
                }
            ...
            ZStack(alignment: .bottomLeading) {
                ...
                if model.detailErrorMessage != nil {
                    Button { model.retryDetail() } label: {
                        MonoIcon(icon: .refresh, size: 16, color: .white)
    ...
    private func previewActions(_ wallpaper: MoeWallsWallpaper) -> some View {
        ...
        return HStack(spacing: 8) {
            Button {
                ...
                HStack(spacing: 8) {
                    if downloading {
                        MonoIcon(icon: .download, size: 14, color: .white)
                    } else {
                        MonoIcon(
            ...
            if existing == nil, model.detail?.fourKDownloadURL != nil {
                Menu {
                    ...
                    MonoIcon(icon: .more, size: 17, color: .white.opacity(0.9))
                        .frame(width: 42, height: 42)
                        .background(
                            RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    private func errorState(_ message: String) -> some View {
        HStack(spacing: 10) {
            MonoIcon(icon: .warning, size: 14, color: .white.opacity(0.62))
            Text(message)
    ...
    private var searchEmptyState: some View {
        VStack(spacing: 10) {
            MonoIcon(icon: .search, size: 22, color: .white.opacity(0.42))
