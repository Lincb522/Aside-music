...
struct OnlineFontLibraryView: View {
    ...
    var body: some View {
        ...
        .fullScreenCover(item: $sourceURL) { url in
            MonoWebView(url: url, title: String(localized: "字体天下"))
        }
    }
    ...
    private var pageHeader: some View {
        HStack(spacing: 12) {
            Button { dismiss() } label: {
                MonoIcon(icon: .chevronLeft, size: 15, color: .white.opacity(0.9))
        ...
        HStack(spacing: 10) {
            ...
            if !viewModel.query.isEmpty {
                Button {
                    ...
                    MonoIcon(icon: .xmarkCircle, size: 15, color: .white.opacity(0.48))
                }
                .buttonStyle(.plain)
            }
        }
    ...
    private func installIndicator(_ item: OnlineFontItem) -> some View {
        Group {
            if viewModel.downloadingID == item.id {
                ...
                MonoIcon(icon: .checkmark, size: 13, color: .black.opacity(0.78))
            } else if !item.canDirectDownload {
                MonoIcon(icon: .chevronRight, size: 13, color: .black.opacity(0.78))
            } else {
                MonoIcon(icon: .download, size: 13, color: .black.opacity(0.78))
            }
        }
