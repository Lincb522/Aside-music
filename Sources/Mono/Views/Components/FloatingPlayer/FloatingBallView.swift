...
struct FloatingBallView: View {
    ...
    private var minimalWhiteCoverDisc: some View {
        if let song = player.currentSong {
            ...
            Circle()
                .fill(MinimalWhiteStyle.controlGlassFill)
                .frame(width: ballSize - 8, height: ballSize - 8)
                .overlay(MonoIcon(icon: .musicNote, size: 16, color: MinimalWhiteStyle.inkMuted))
                .overlay(Circle().stroke(MinimalWhiteStyle.hairline, lineWidth: MinimalWhiteStyle.strokeWidth))
        }
    }
    ...
    private var asideCoverOrb: some View {
        ZStack {
            if let song = player.currentSong {
                CachedAsyncImage(url: song.coverUrl) {
                    Circle().fill(Color.monoTextPrimary.opacity(0.08))
                }
                .aspectRatio(contentMode: .fill)
            } else {
                Circle()
                    .fill(Color.monoTextPrimary.opacity(0.07))
                    .overlay(MonoIcon(icon: .musicNote, size: 15, color: .monoTextSecondary.opacity(0.62), lineWidth: 1.6))
            }
            ...
            if player.currentSong != nil {
                Circle()
                    ...
                    .frame(width: 10, height: 10)
                    .overlay(Circle().fill(Color.monoTextPrimary.opacity(0.85)).frame(width: 3.5, height: 3.5))
            }
        }
        .clipShape(Circle())
    }
        ...
        ZStack {
            ...
            if let song = player.currentSong {
                ...
                Circle()
                    ...
                    .frame(width: 22, height: 22)
                    .overlay(
                        MonoIcon(icon: .musicNote, size: 10, color: emptyCoverIconColor)
                    )
            }
    ...
    private var panelBackground: some View {
        if MinimalWhiteStyle.isActive {
            ...
            if !ThemedPageStyle.isActive && settings.defaultThemeUsesLiquidGlassTabBar {
                RoundedRectangle(cornerRadius: panelCornerRadius, style: .continuous)
                    .fill(Color.monoFloatingBarFill)
                    .monoGlass(cornerRadius: panelCornerRadius)
            } else {
                RoundedRectangle(cornerRadius: panelCornerRadius, style: .continuous)
                    .fill(ThemedPageStyle.isActive ? AnyShapeStyle(Color.clear) : AnyShapeStyle(.regularMaterial))
                    .overlay {
    ...
    private var tabSection: some View {
        let items: [(tab: Tab, outline: MonoIcon.IconType, filled: MonoIcon.IconType)] = [
    ...
    private var progressFill: AnyShapeStyle {
        if settings.globalThemeId == .default {
            return AnyShapeStyle(Color.monoAccent)
        }
        if MinimalWhiteStyle.isActive {
        ...
        return AnyShapeStyle(Color.monoAccent.opacity(0.6))
    }
    ...
    private func tabSelectionBackground(_ tab: Tab) -> some View {
        if MinimalWhiteStyle.isActive {
            ...
            Circle()
                .fill(Color.monoAccent.opacity(colorScheme == .dark ? 0.18 : 0.13))
                .overlay(Circle().stroke(Color.monoAccent.opacity(colorScheme == .dark ? 0.32 : 0.24), lineWidth: 0.7))
        }
    }
