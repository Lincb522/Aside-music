...
struct ClassicFloatingBar: View {
                ...
                .overlay(
                    shape.fill(
                        settings.defaultThemeUsesLiquidGlassTabBar
                            ? Color.monoFloatingBarFill.opacity(colorScheme == .dark ? 0.58 : 0.50)
                ...
                .overlay(
                    LinearGradient(
                        colors: [
                            ...
                            Color.clear,
                            Color.monoAccent.opacity(colorScheme == .dark ? 0.04 : 0.035),
                        ],
                        startPoint: .topLeading,
                        endPoint: .bottomTrailing
                    )
    ...
    private var dockAccentRail: some View {
        ...
        case .default:
            Capsule()
                .fill(Color.monoAccent.opacity(colorScheme == .dark ? 0.44 : 0.28))
        ...
        VStack(spacing: 6) {
            HStack(spacing: 10) {
                ...
                Button(action: { showPlaylist.toggle() }) {
                    MonoIcon(icon: .list, size: 14, color: MinimalWhiteStyle.inkMuted, lineWidth: 1.7)
                        .frame(width: 32, height: 32)
                }
                .buttonStyle(MonoBouncingButtonStyle(scale: 0.94))
            }
...
private struct ClassicMiniPlayerSection: View {
        ...
        if MangaStyle.isActive {
            ...
            shape
                ...
                .overlay(
                    shape.fill(
                        settings.defaultThemeUsesLiquidGlassTabBar
                            ? Color.monoFloatingBarFill.opacity(colorScheme == .dark ? 0.56 : 0.46)
    ...
    @ViewBuilder
    private func classicBarIcon(icon: MonoIcon.IconType, size: CGFloat, color: Color, lineWidth: CGFloat) -> some View {
    ...
    private var secondaryControlFill: Color {
        ...
        return Color.monoTextPrimary.opacity(0.07)
    }
    ...
    private var closeFill: Color {
        ...
        if BentoStyle.isActive { return BentoStyle.ink.opacity(0.06) }
        return Color.monoTextPrimary.opacity(0.08)
    }
...
private struct ClassicTabBarSection: View {
    ...
    private func tabSelectionBackground(_ index: Int) -> some View {
        if MangaStyle.isActive {
            ...
            RoundedRectangle(cornerRadius: 17, style: .continuous)
                .fill(Color.monoAccent.opacity(colorScheme == .dark ? 0.16 : 0.12))
                .overlay(
                    RoundedRectangle(cornerRadius: 17, style: .continuous)
                        .stroke(Color.monoAccent.opacity(colorScheme == .dark ? 0.30 : 0.22), lineWidth: 0.7)
                )
                .padding(.horizontal, 4)
        }
    }
            ...
            .overlay(
                LinearGradient(
                    colors: [
                        ...
                        Color.clear,
                        Color.monoAccent.opacity(colorScheme == .dark ? 0.04 : 0.025),
                    ],
                    startPoint: .top,
                    endPoint: .bottom
                )
...
private struct AsideClassicNowPlayingRow: View {
        ...
        HStack(spacing: 10) {
            CachedAsyncImage(url: song.coverUrl, width: 38, height: 38) {
                RoundedRectangle(cornerRadius: 11, style: .continuous)
                    ...
                    .overlay(MonoIcon(icon: .musicNote, size: 14, color: .monoTextSecondary.opacity(0.6), lineWidth: 1.5))
            }
            .aspectRatio(contentMode: .fill)
            ...
            HStack(spacing: 8) {
                ...
                Button(action: { showPlaylist.toggle() }) {
                    MonoIcon(icon: .list, size: 14, color: .monoTextPrimary.opacity(0.66), lineWidth: 1.7)
                        .frame(width: 30, height: 30)
                        .background(Circle().strokeBorder(Color.monoTextPrimary.opacity(0.13), lineWidth: 1))
                        .contentShape(Circle())
                }
                .buttonStyle(MonoBouncingButtonStyle(scale: 0.93))
                ...
                if !player.isPlaying {
                    Button {
                        ...
                        MonoIcon(icon: .close, size: 9.5, color: .monoTextSecondary, lineWidth: 1.6)
    ...
    private func sourceBadge(icon: MonoIcon.IconType) -> some View {
        MonoIcon(icon: icon, size: 9, color: .monoAccentForeground, lineWidth: 1.6)
        ...
        return Button {
            ...
            VStack(spacing: 3) {
                ...
                Text(label)
                    ...
                    .foregroundColor(selected ? .monoTextPrimary : .monoTextSecondary.opacity(0.55))
                    .lineLimit(1)
                    .minimumScaleFactor(0.78)
