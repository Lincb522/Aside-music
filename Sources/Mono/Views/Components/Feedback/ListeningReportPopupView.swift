            ...
            VStack(spacing: 0) {
                HStack {
                    ...
                    Button(action: onDismiss) {
                        MonoIcon(icon: .close, size: 10, color: .white)
        ...
        ZStack {
            LinearGradient(
                colors: [
                    ...
                    Color.monoTextPrimary.opacity(0.82),
                ],
                startPoint: .topLeading,
                endPoint: .bottomTrailing
            )
            MonoIcon(icon: .waveform, size: 62, color: .white.opacity(0.18), lineWidth: 1.2)
        }
        .frame(maxWidth: .infinity)
        .frame(height: 238)
    }
                        ...
                        RoundedRectangle(cornerRadius: 2.5, style: .continuous)
                            .fill(
                                bucket.date == visiblePeakDate
                                    ? Color.monoAccent
                                    : Color.monoTextPrimary.opacity(0.24)
                            )
        ...
        VStack(spacing: 11) {
            Button(action: onOpenReport) {
                HStack {
                    ...
                    Spacer()
                    MonoIcon(icon: .chevronRight, size: 10, color: Color(light: .white, dark: .black))
                }
                ...
                .frame(maxWidth: .infinity)
                .frame(height: 48)
                .background(RoundedRectangle(cornerRadius: 14, style: .continuous).fill(Color.monoAccent))
            }
