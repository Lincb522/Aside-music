...
struct QishuiQualityPickerSheet: View {
                        ...
                        Button(action: { onSelect(item.key) }) {
                            HStack(spacing: 14) {
                                ZStack {
                                    ...
                                    MonoIcon(icon: .soundQuality, size: 16, color: currentQuality == item.key ? selectedIconColor : defaultIconColor)
                                }
                                ...
                                VStack(alignment: .leading, spacing: 2) {
                                    HStack(spacing: 6) {
                                        Text(item.name)
                                            ...
                                            .foregroundColor(SequoiaStyle.isActive ? SequoiaStyle.ink : .monoTextPrimary)
                                    ...
                                    Text(item.subtitle)
                                        ...
                                        .foregroundColor(SequoiaStyle.isActive ? SequoiaStyle.inkSoft : .monoTextSecondary)
                                }

                                Spacer()

                                if currentQuality == item.key {
                                    MonoIcon(icon: .checkmark, size: 14, color: NeumorphicStyle.isActive ? NeumorphicStyle.accent : (SequoiaStyle.isActive ? SequoiaStyle.accent : .monoTextPrimary))
                                }
                            }
    ...
    private var selectedIconColor: Color {
        ...
        return NeumorphicStyle.isActive ? NeumorphicStyle.accent : .monoIconForeground
    }
    ...
    private var qualityBadgeForeground: Color {
        ...
        return .monoTextPrimary.opacity(0.72)
    }
    ...
    private var qualityBadgeBackground: some View {
        if SequoiaStyle.isActive {
            ...
            Capsule().fill(Color.monoSeparator.opacity(0.55))
        }
    }
    ...
    private var qualityBadgeStroke: some View {
        if SequoiaStyle.isActive {
            ...
            Capsule().stroke(Color.monoTextPrimary.opacity(0.32), lineWidth: 0.8)
        } else {
            Capsule().stroke(Color.monoTextSecondary.opacity(0.08), lineWidth: 0.6)
        }
    }
    ...
    private func qualityIconTileBackground(isSelected: Bool) -> some View {
        if NeumorphicStyle.isActive {
            ...
            RoundedRectangle(cornerRadius: 8, style: .continuous)
                .fill(isSelected ? Color.monoIconBackground : Color.monoIconBackground.opacity(0.08))
        }
    }
}
