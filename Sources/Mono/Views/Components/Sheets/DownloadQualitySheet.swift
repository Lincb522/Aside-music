...
struct DownloadQualitySheet: View {
    ...
    /// qcm 未上报高级档位时仍保留入口，最终可用性由实际 URL 请求确认。
    ...
    private var header: some View {
        HStack {
            ...
            Button(action: { dismissCurrentPresentation(systemDismiss: dismiss, monoSheetDismiss: monoSheetDismiss) }) {
                MonoIcon(icon: .close, size: 14, color: headerSubtitleColor)
                    .padding(10)
    ...
    private func qualityRow(name: String, subtitle: String, badge: String?, isLocked: Bool = false) -> some View {
        HStack(spacing: 14) {
            ZStack {
                ...
                MonoIcon(icon: isLocked ? .lock : .playerDownload, size: 16, color: qualityIconColor(isLocked: isLocked))
            }
            ...
            if !isLocked {
                MonoIcon(icon: .playerDownload, size: 14, color: qualitySubtitleColor)
            }
        }
        .padding(.horizontal, 16)
        .padding(.vertical, 12)
    ...
    private func qualityIconTileBackground(isLocked: Bool) -> some View {
        if NeumorphicStyle.isActive {
            ...
            RoundedRectangle(cornerRadius: 8, style: .continuous)
                .fill(isLocked ? Color.monoIconBackground.opacity(0.04) : Color.monoIconBackground.opacity(0.08))
        }
    }

    private func qualityIconColor(isLocked: Bool) -> Color {
        if isLocked { return SequoiaStyle.isActive ? SequoiaStyle.inkMuted.opacity(0.5) : .monoTextSecondary.opacity(0.4) }
    ...
    private func qualityBadgeForeground(isLocked: Bool) -> Color {
        ...
        if isAsideTheme { return .monoTextPrimary.opacity(0.88) }
        return .monoIconForeground
    }

    @ViewBuilder
    private func qualityBadgeBackground(isLocked: Bool) -> some View {
        let tint = MangaStyle.isActive ? MangaStyle.labelYellow : (MujiStyle.isActive ? MujiStyle.clay : (NeumorphicStyle.isActive ? NeumorphicStyle.accent : (SequoiaStyle.isActive ? SequoiaStyle.accent : Color.monoIconBackground)))
        if isLocked {
            RoundedRectangle(cornerRadius: qualityBadgeCornerRadius, style: .continuous)
                .fill(Color.monoIconBackground.opacity(0.3))
    ...
    private func qualityBadgeStroke(isLocked: Bool) -> some View {
        ...
        if MangaStyle.isActive {
            ...
            Capsule()
                .stroke(Color.monoTextPrimary.opacity(isLocked ? 0.14 : 0.32), lineWidth: 0.8)
        }
    }
}
