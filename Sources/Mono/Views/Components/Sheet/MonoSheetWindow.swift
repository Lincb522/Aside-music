...
@MainActor
final class MonoSheetWindow {
    static let shared = MonoSheetWindow()

    private var window: MonoSheetPassthroughWindow?
    ...
    func setup() {
        ...
        let overlayWindow = MonoSheetPassthroughWindow(windowScene: windowScene)
        overlayWindow.windowLevel = .alert + 1
        overlayWindow.backgroundColor = .clear

        let hostingController = UIHostingController(rootView: MonoSheetWindowContent())
        ...
        cancellable = MonoSheetManager.shared.$entries
            .receive(on: RunLoop.main)
            .sink { [weak overlayWindow] entries in
                overlayWindow?.sheetActive = !entries.isEmpty
            }

        overlayWindow.sheetActive = MonoSheetManager.shared.hasActiveSheet
    }
...
private final class MonoSheetPassthroughWindow: UIWindow {
...
private struct MonoSheetWindowContent: View {
    @ObservedObject private var sheetManager = MonoSheetManager.shared
    @ObservedObject private var settings = SettingsManager.shared

    var body: some View {
        ZStack {
            ForEach(Array(sheetManager.entries.enumerated()), id: \.element.id) { index, entry in
                MonoPresentedSheetView(
                    entry: entry,
...
private struct MonoPresentedSheetView: View {
    let entry: MonoSheetEntry
    let isTopmost: Bool

    @StateObject private var dragCoordinator = MonoSheetDragCoordinator()
    ...
    var body: some View {
        ...
        .animation(MonoSheetAnimation.present, value: entry.isVisible)
        .animation(MonoSheetAnimation.present, value: isTopmost)
    ...
    private var activeDragCoordinator: MonoSheetDragCoordinator? {
...
private struct MonoStaticSheetContentHost: View, Equatable {
    let version: Int
    let themeRevision: Int
    let content: AnyView

    nonisolated static func == (lhs: MonoStaticSheetContentHost, rhs: MonoStaticSheetContentHost) -> Bool {
