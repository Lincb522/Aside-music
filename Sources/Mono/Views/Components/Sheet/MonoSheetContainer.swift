...
struct MonoSheetContainer<Content: View>: View {
    let preset: MonoSheetPreset
    let stretchAmount: CGFloat
    let dragCoordinator: MonoSheetDragCoordinator?
    ...
    init(
        ...
        dragCoordinator: MonoSheetDragCoordinator? = nil,
    ...
    var body: some View {
        ...
        GeometryReader { proxy in
            ...
            let bottomSurfaceExtension = MonoSheetThemeStyle.attachesSurfaceToBottom ? bottomInset : 0
            let visualPanelHeight = panelHeight + bottomSurfaceExtension + max(0, stretchAmount)
            let outsideBottomPadding = MonoSheetThemeStyle.attachesSurfaceToBottom ? 0 : bottomInset
            ...
            VStack(spacing: 0) {
                ...
                // 内容通过 .monoSheetSurface 上抛的整面背景铺满整个面板
                // （含把手区），把手区不再露出一截默认毛玻璃。
                .backgroundPreferenceValue(MonoSheetSurfacePreferenceKey.self) { customSurface in
                    ZStack {
                        MonoSheetSurfaceBackground(cornerRadius: resolvedCornerRadius)
                ...
                .modifier(
                    MonoSheetLiquidSurfaceModifier(
                        cornerRadius: resolvedCornerRadius,
                        isEnabled: !isInteractiveMotionActive && !MonoSheetThemeStyle.usesCustomThemeSurface
                    )
                )
                .overlayPreferenceValue(MonoSheetSurfacePreferenceKey.self) { customSurface in
                    MonoSheetSurfaceOverlay(
                        cornerRadius: resolvedCornerRadius,
                ...
                .shadow(
                    color: MonoSheetThemeStyle.shadowColor,
                    radius: MonoSheetThemeStyle.shadowRadius(
                        colorScheme: colorScheme,
                        isInteractiveMotionActive: isInteractiveMotionActive
                    ),
                    x: 0,
                    y: MonoSheetThemeStyle.shadowYOffset(isInteractiveMotionActive: isInteractiveMotionActive)
                )
    ...
    private var panelShape: MonoSheetSurfaceShape {
        MonoSheetSurfaceShape(
            cornerRadius: resolvedCornerRadius,
            attachesToBottom: MonoSheetThemeStyle.attachesSurfaceToBottom
        )
    }

    private var resolvedCornerRadius: CGFloat {
        preset.monoResolvedCornerRadius
    }
...
struct MonoSheetCustomSurface: Equatable, @unchecked Sendable {
    let id: String
    let view: AnyView

    static func == (lhs: MonoSheetCustomSurface, rhs: MonoSheetCustomSurface) -> Bool {
        lhs.id == rhs.id
    }
}

struct MonoSheetSurfacePreferenceKey: PreferenceKey {
    static let defaultValue: MonoSheetCustomSurface? = nil

    static func reduce(
        value: inout MonoSheetCustomSurface?,
        nextValue: () -> MonoSheetCustomSurface?
    ) {
...
extension View {
    /// 声明这张 sheet 的整面背景。
    /// 与直接 `.background(...)` 的区别：背景交给 monoSheet 容器绘制，
    /// 铺满整个面板（包括顶部把手区域），避免把手处露出默认毛玻璃。
    func monoSheetSurface<S: View>(
        id: String,
        @ViewBuilder _ surface: () -> S
    ) -> some View {
        preference(
            key: MonoSheetSurfacePreferenceKey.self,
            value: MonoSheetCustomSurface(id: id, view: AnyView(surface()))
        )
    }
}

private struct MonoSheetLiquidSurfaceModifier: ViewModifier {
...
struct MonoSheetSurfaceShape: InsettableShape {
    let cornerRadius: CGFloat
    ...
    func inset(by amount: CGFloat) -> MonoSheetSurfaceShape {
        var shape = self
...
struct MonoSheetHandleView: View {
    @Environment(\.colorScheme) private var colorScheme

    var body: some View {
        if MonoSheetThemeStyle.usesIntegratedHandle {
            Capsule(style: .continuous)
                .fill(Color.monoSheetHandle.opacity(colorScheme == .dark ? 0.76 : 0.62))
            ...
            Capsule()
                .fill(Color.monoSheetHandle)
                .frame(width: 42, height: 5)
        }
    }
}

struct MonoSheetSurfaceBackground: View {
    let cornerRadius: CGFloat

    @Environment(\.colorScheme) private var colorScheme

    var body: some View {
        let shape = MonoSheetSurfaceShape(
        ...
        ZStack {
            if MinimalWhiteStyle.isActive {
                ...
                shape
                    .fill(
                        LinearGradient(
                            colors: [Color.monoSheetSurfaceTop, Color.monoSheetSurfaceBottom],
                            startPoint: .topLeading,
                            endPoint: .bottomTrailing
                        )
                    )
                ...
                shape
                    .fill(
                        RadialGradient(
                            colors: [Color.monoSheetInnerGlow, .clear],
                            center: .topLeading,
                            startRadius: 0,
                            endRadius: 280
                        )
...
struct MonoSheetSurfaceOverlay: View {
    let cornerRadius: CGFloat
    ...
    var body: some View {
        ...
        ZStack(alignment: .top) {
            if MinimalWhiteStyle.isActive {
                ...
                shape
                    .strokeBorder(Color.monoSheetStroke, lineWidth: 1)

                if showsHighlight {
                    LinearGradient(
                        colors: [Color.monoSheetHighlight, .clear],
                        startPoint: .top,
                        endPoint: .bottom
                    )
                    .frame(height: 120)
