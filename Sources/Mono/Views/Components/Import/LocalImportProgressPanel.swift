...
/// 本地音乐导入/扫描的悬浮进度面板：状态图标 + 阶段文案 + 进度条，
/// 完成后展示导入/跳过/失败的统计胶囊。
struct LocalImportProgressPanel: View {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: ThemedPageStyle.isActive ? 8 : 12, style: .continuous)
                .fill(Color.monoGlassTint.opacity(0.66))
                .monoGlass(cornerRadius: ThemedPageStyle.isActive ? 8 : 12)
        )
    ...
    private var statusIcon: some View {
        ZStack {
            ...
            if progress.isCompleted {
                MonoIcon(icon: .checkmark, size: 13, color: accent, lineWidth: 1.8)
            } else {
                ProgressView()
                    .tint(accent)
                    .scaleEffect(0.58)
    ...
    private var completedSummary: some View {
        HStack(spacing: 6) {
            metricPill(title: String(localized: "local_toolbar_import"), value: progress.importedCount)
