...
private extension ArtistDetailView {
    @ViewBuilder
    var artistPlatformBadge: some View {
        if let source = viewModel.artist?.source, source == .appleMusic || source == .kugou {
            PlatformBadgeLabel(
            ...
            VStack(spacing: 13) {
                CachedAsyncImage(url: coverURL) {
                    Circle()
                        ...
                        .overlay(MonoIcon(icon: .profile, size: 38, color: MinimalWhiteStyle.inkMuted, lineWidth: 1.55))
                }
                .aspectRatio(contentMode: .fill)
...
extension ArtistDetailView {
    ...
    private var neumorphicArtistCoverStack: some View {
        ZStack(alignment: .bottomTrailing) {
            ...
            MonoIcon(icon: .sparkle, size: 14, color: NeumorphicStyle.warm, lineWidth: 1.75)
    ...
    private func neumorphicArtistMetricTile(title: String, value: String, icon: MonoIcon.IconType, tint: Color) -> some View {
        HStack(spacing: 9) {
    ...
    private func neumorphicArtistTabDockItem(_ title: String, index: Int, icon: MonoIcon.IconType, tint: Color) -> some View {
...
extension ArtistDetailView {
                ...
                Button(action: { showFullDescription = true }) {
                    HStack(spacing: 8) {
                        Text(desc)
                            ...
                            .lineLimit(2)
                        Spacer(minLength: 6)
                        MonoIcon(icon: .chevronRight, size: 10, color: MinimalWhiteStyle.inkMuted, lineWidth: 1.5)
                    }
                }
                .buttonStyle(.plain)
            }
                ...
                Button(action: { showFullDescription = true }) {
                    HStack(spacing: 6) {
                        Text(desc)
                            ...
                            .lineLimit(2)
                        MonoIcon(icon: .chevronRight, size: 10, color: MangaStyle.inkSub)
                    }
                }
                .buttonStyle(.plain)
            }
    ...
    private func neumorphicArtistNote(text: String, showsChevron: Bool) -> some View {
        HStack(alignment: .center, spacing: 8) {
            ...
            if showsChevron {
                MonoIcon(icon: .chevronRight, size: 10, color: NeumorphicStyle.inkSoft)
            }
        }
                ...
                Button(action: { showFullDescription = true }) {
                    HStack(spacing: 6) {
                        Text(desc)
                            ...
                            .lineLimit(2)
                        MonoIcon(icon: .chevronRight, size: 10, color: SignalStyle.inkSoft)
                    }
                }
                .buttonStyle(.plain)
            }
        ...
        BentoBlock(fill: BentoStyle.surfaceRaised, radius: BentoStyle.blockRadiusLarge, padding: 16, stroked: true) {
            VStack(alignment: .leading, spacing: 13) {
                ...
                Button(action: {
                    ...
                    HStack(spacing: 8) {
                        MonoIcon(icon: .play, size: 13, color: BentoStyle.onAccent, lineWidth: 2)
                        Text(LocalizedStringKey("artist_play_all"))
...
extension ArtistDetailView {
    ...
    private var capsuleArtistHeader: some View {
        ...
        return CapsuleDetailHeader(
            ...
            title: artist?.name ?? String(localized: "lib_tab_artists"),
    ...
    private func tabItem(_ title: String, index: Int) -> some View {
        Button(action: {
            ...
            VStack(spacing: 6) {
                ...
                Capsule()
                    .fill(selectedTab == index ? Color.monoIconBackground : Color.clear)
...
extension ArtistDetailView {
    ...
    private var artistSongRows: some View {
        LazyVStack(spacing: (CapsuleStyle.isActive || MinimalWhiteStyle.isActive) ? 4 : 0) {
            ForEach(Array(artistSongFiltered.enumerated()), id: \.element.id) { index, song in
                SongListRow(song: song, index: index, isSelecting: isArtistSelectMode, isSelected: artistSelectedSongIds.contains(song.id), onArtistTap: initialArtist?.source == .appleMusic || initialArtist?.source == .kugou ? nil : { artistId in
                    selectedArtistInfo = nil
                        ...
                        Button(action: {
                            ...
                            VStack(spacing: 10) {
                                if let coverUrl = artist.coverUrl?.sized(300) {
                                    CachedAsyncImage(url: coverUrl) {
                                        RoundedRectangle(cornerRadius: MangaStyle.isActive ? MangaStyle.cardRadius : (MujiStyle.isActive ? 12 : (SignalStyle.isActive ? 24 : (NeumorphicStyle.isActive ? 24 : 45))))
                                            .fill(MangaStyle.isActive ? MangaStyle.paperCool : (MujiStyle.isActive ? MujiStyle.surfaceRaised : (SignalStyle.isActive ? SignalStyle.controlPressed : (NeumorphicStyle.isActive ? NeumorphicStyle.surfacePressed : (SequoiaStyle.isActive ? SequoiaStyle.materialList : Color.monoGlassTint)))))
                                    }
                                    .aspectRatio(contentMode: .fill)
                                    ...
                                    RoundedRectangle(cornerRadius: MangaStyle.isActive ? MangaStyle.cardRadius : (MujiStyle.isActive ? 12 : (SignalStyle.isActive ? 24 : (NeumorphicStyle.isActive ? 24 : 45))))
                                        .fill(MangaStyle.isActive ? MangaStyle.paperCool : (MujiStyle.isActive ? MujiStyle.surfaceRaised : (SignalStyle.isActive ? SignalStyle.controlPressed : (NeumorphicStyle.isActive ? NeumorphicStyle.surfacePressed : (SequoiaStyle.isActive ? SequoiaStyle.materialList : Color.monoGlassTint)))))
                                        .frame(width: 90, height: 90)
                                        .overlay(MonoIcon(icon: .personCircle, size: 32, color: MangaStyle.isActive ? MangaStyle.inkSub : (MujiStyle.isActive ? MujiStyle.inkMuted : (SignalStyle.isActive ? SignalStyle.inkSoft : (NeumorphicStyle.isActive ? NeumorphicStyle.inkSoft : (SequoiaStyle.isActive ? SequoiaStyle.inkMuted : .monoTextSecondary.opacity(0.3)))))))
                                }
                                ...
                                Text(artist.name)
                                    ...
                                    .foregroundColor(MangaStyle.isActive ? MangaStyle.ink : (MujiStyle.isActive ? MujiStyle.ink : (SignalStyle.isActive ? SignalStyle.ink : (NeumorphicStyle.isActive ? NeumorphicStyle.ink : (SequoiaStyle.isActive ? SequoiaStyle.ink : .monoTextPrimary)))))
                                    .lineLimit(1)
                            }
            ...
            VStack(spacing: 9) {
                CachedAsyncImage(url: artist.coverUrl?.sized(300), width: 78, height: 78) {
                    RoundedRectangle(cornerRadius: 26, style: .continuous)
                        ...
                        .overlay(MonoIcon(icon: .personCircle, size: 28, color: CapsuleStyle.mint, lineWidth: 1.7))
                }
                .aspectRatio(contentMode: .fill)
        ...
        Button {
            ...
            VStack(spacing: 9) {
                CachedAsyncImage(url: artist.coverUrl?.sized(300), width: 78, height: 78) {
                    RoundedRectangle(cornerRadius: 24, style: .continuous)
                        .fill(MinimalWhiteStyle.controlGlassFill)
                        .overlay(MonoIcon(icon: .personCircle, size: 28, color: MinimalWhiteStyle.inkMuted, lineWidth: 1.7))
                }
                .aspectRatio(contentMode: .fill)
                .frame(width: 78, height: 78)
                .clipShape(RoundedRectangle(cornerRadius: 24, style: .continuous))
    ...
    private var loadingPlaceholder: some View {
        VStack(spacing: 14) {
            ...
            ProgressView()
                .progressViewStyle(CircularProgressViewStyle(tint: MinimalWhiteStyle.isActive ? MinimalWhiteStyle.inkMuted : (SignalStyle.isActive ? SignalStyle.accent : (NeumorphicStyle.isActive ? NeumorphicStyle.accent : (SequoiaStyle.isActive ? SequoiaStyle.accent : .monoTextSecondary)))))
    ...
    private func emptyPlaceholder(_ text: String) -> some View {
        VStack(spacing: 14) {
            ...
            Text(text)
                ...
                .foregroundColor(MinimalWhiteStyle.isActive ? MinimalWhiteStyle.inkMuted : (SignalStyle.isActive ? SignalStyle.inkSoft : (NeumorphicStyle.isActive ? NeumorphicStyle.inkSoft : (CapsuleStyle.isActive ? CapsuleStyle.inkSoft : (SequoiaStyle.isActive ? SequoiaStyle.inkSoft : .monoTextSecondary)))))
            Spacer().frame(height: 48)
        }
        .frame(maxWidth: .infinity)
        .background {
