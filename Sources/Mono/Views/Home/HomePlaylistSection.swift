...
struct HomeNCMPlaylistSection: View {
    ...
    var title = NSLocalizedString("playlists_love", comment: "")
    var subtitle = NSLocalizedString("based_on_taste", comment: "")
        ...
        return VStack(alignment: .leading, spacing: 0) {
            ZStack(alignment: .topLeading) {
                CachedAsyncImage(url: playlist.coverUrl?.sized(400), width: cardSize, height: cardSize) {
                    Rectangle()
                        .fill(MinimalWhiteStyle.isActive ? MinimalWhiteStyle.controlGlassFill : (NeumorphicStyle.isActive ? NeumorphicStyle.surfacePressed : Color.monoSeparator))
                }
                ...
                if let count = playlist.playCount, count > 0 {
                    HStack(spacing: 3) {
                        MonoIcon(icon: .play, size: 7, color: .white)
