...
struct MVEmbeddedCommentSection: View {
            ...
            ForEach(CommentSortType.allCases, id: \.rawValue) { type in
                Button {
                    ...
                    Text(type.title)
                        ...
                        .foregroundColor(commentVM.sortType == type ? .monoIconForeground : .monoTextSecondary)
                        .padding(.horizontal, 14)
                        .padding(.vertical, 7)
                        .background(
                            Capsule()
                                .fill(commentVM.sortType == type ? Color.monoIconBackground : Color.monoTextPrimary.opacity(0.05))
                        )
                }
                .buttonStyle(.plain)
            }
    ...
    private var emptyView: some View {
        VStack(spacing: 12) {
            MonoIcon(icon: .comment, size: 32, color: .monoTextSecondary.opacity(0.25))
        ...
        VStack(spacing: 14) {
            ...
            if !commentVM.hotComments.isEmpty {
                VStack(alignment: .leading, spacing: 10) {
                    HStack(spacing: 6) {
                        MonoIcon(icon: .sparkle, size: 14, color: .monoOrange)
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if let reply = commentVM.replyTarget {
                HStack(spacing: 8) {
                    ...
                    Button {
                        ...
                        MonoIcon(icon: .xmark, size: 10, color: .monoTextSecondary)
                            .padding(6)
            ...
            HStack(spacing: 12) {
                HStack(spacing: 8) {
                    TextField(
                        commentVM.replyTarget.map {
                            L10n.format("comment_reply_placeholder", $0.user.nickname)
                        } ?? String(localized: "comment_placeholder"),
                    ...
                    if !commentVM.commentText.isEmpty {
                        Button {
                            ...
                            MonoIcon(icon: .xmark, size: 10, color: .monoTextSecondary)
                                .padding(4)
                                .background(Circle().fill(Color.monoTextPrimary.opacity(0.08)))
                        }
                    }
                }
                ...
                Button {
                    ...
                    ZStack {
                        Circle()
                            .fill(canSend ? Color.monoIconBackground : Color.monoTextPrimary.opacity(0.06))
                            .frame(width: 36, height: 36)
                        if commentVM.isSending {
                            ProgressView()
                                .scaleEffect(0.7)
                                .tint(.monoIconForeground)
                        } else {
                            MonoIcon(
                                icon: .send,
                                size: 16,
                                color: canSend ? .monoIconForeground : .monoTextSecondary.opacity(0.4)
                            )
                        }
                    }
                }
