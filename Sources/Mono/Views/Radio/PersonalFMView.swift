...
struct PersonalFMView: View {
    ...
    private struct Theme {
        ...
        static var cardBackground: Color {
            ...
            return Color.monoGlassTint.opacity(0.8)
        }
            ...
            VStack(spacing: 0) {
                ...
                ZStack {
                    if let song = currentFMSong {
                            ...
                            ZStack {
                                CachedAsyncImage(url: song.coverUrl) {
                                    Theme.pressedBackground.opacity(0.72).overlay(
                                        MonoIcon(icon: .fm, size: 80, color: Theme.accent.opacity(0.12))
                                    )
                                }
                        ...
                        .overlay(
                            ZStack {
                                if dragOffset.width < -50 {
                                    ...
                                    MonoIcon(icon: .trash, size: 32, color: .white)
                                }
                            }
                        ...
                        .overlay(
                            ZStack {
                                if dragOffset.width > 50 {
                                    ...
                                    MonoIcon(icon: .like, size: 32, color: .white)
                                }
                            }
                            .opacity(Double(min(abs(dragOffset.width) / 150, 1.0)))
                            .offset(x: -60)
                        ...
                        MonoLoadingView(text: "LOADING STATIONS")
                    } else {
                        emptyStateView()
                    }
                }
            ...
            VStack {
                HStack(alignment: .center) {
                    ...
                    if PetWhiteStyle.isActive {
                        ...
                        Button(action: { showFMModePicker = true }) {
                            MonoIcon(icon: .fmMode, size: 20, color: Theme.accent)
                                .frame(width: 44, height: 44)
                        }
                    }
                }
    ...
    private var defaultFMControlsBar: some View {
        HStack(spacing: 40) {
            if let song = currentFMSong {
                ...
                Button(action: {}) {
                    MonoIcon(icon: .like, size: 24, color: Theme.accent)
                        .frame(width: 50, height: 50)
                        .background(Circle().fill(Theme.pressedBackground))
                        .monoGlassCircle()
                }
            ...
            Button(action: {
                ...
                ZStack {
                    Circle()
                        .fill((NeumorphicStyle.isActive || SequoiaStyle.isActive) ? Theme.accent : Color.monoGlassTint)
                        .frame(width: 72, height: 72)
                        .monoGlassCircle()
                        .shadow(color: Color.black.opacity(SequoiaStyle.isActive ? 0.08 : 0.15), radius: SequoiaStyle.isActive ? 14 : 10, x: 0, y: 5)

                    MonoIcon(icon: isFMPlaying ? .pause : .play, size: 26, color: (NeumorphicStyle.isActive || SequoiaStyle.isActive) ? Theme.accentForeground : Theme.accent)
                        .offset(x: isFMPlaying ? 0 : 2)
                }
            }
            ...
            Button(action: {
                ...
                MonoIcon(icon: .next, size: 24, color: Theme.accent)
                    .frame(width: 50, height: 50)
                    .background(Circle().fill(Theme.pressedBackground))
                    .monoGlassCircle()
            }
    ...
    private var defaultEmptyStateView: some View {
        VStack(spacing: 24) {
            MonoIcon(icon: .fm, size: 40, color: Theme.accent.opacity(0.18))
            ...
            Button(action: { loadFMData() }) {
                Text(LocalizedStringKey("action_retry"))
                    ...
                    .padding(.vertical, 12)
                    .background(RoundedRectangle(cornerRadius: 12).fill((NeumorphicStyle.isActive || SequoiaStyle.isActive) ? Theme.accent : Color.monoIconBackground))
            }
        }
    }
