...
struct ServerLineSelectorView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 10) {
                MonoIcon(icon: .cloud, size: 14, color: statusColor)
    ...
    private func optionButton(_ option: ServerLinePreference) -> some View {
        ...
        return Button {
            ...
            Text(optionTitle(option))
                ...
                .foregroundColor(isSelected ? Color(light: .white, dark: .black) : .monoTextPrimary)
                .frame(maxWidth: .infinity)
                .padding(.vertical, 9)
                .background(
                    RoundedRectangle(cornerRadius: 10, style: .continuous)
                        .fill(isSelected ? Color.monoAccent : Color.monoSeparator.opacity(0.32))
                )
        }
