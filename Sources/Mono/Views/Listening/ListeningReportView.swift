...
struct ListeningReportView: View {
    ...
    private var floatingTopBar: some View {
        HStack {
            ...
            Button { dismiss() } label: {
                MonoIcon(icon: .close, size: 11, color: .monoTextPrimary)
    ...
    private var coverFallback: some View {
        ZStack {
            LinearGradient(
                colors: [
                    Color.monoAccent.opacity(0.86),
                    Color.monoTextPrimary.opacity(0.86),
                ],
                startPoint: .topLeading,
                endPoint: .bottomTrailing
            )
            MonoIcon(icon: .waveform, size: 86, color: .white.opacity(0.17), lineWidth: 1.2)
        }
    }
    ...
    private func periodButton(
        ...
        Button(action: action) {
            MonoIcon(icon: icon, size: 12, color: enabled ? .white : .white.opacity(0.28))
    ...
    private func comparisonLabel(_ change: Int) -> some View {
        ...
        .frame(height: 28)
        .background(Capsule().fill(Color.monoAccent.opacity(0.11)))
    }
                ...
                ForEach(Array(report.buckets.enumerated()), id: \.element.id) { index, bucket in
                    ...
                    VStack(spacing: 8) {
                        ...
                        Text(showLabel ? ListeningReportFormatter.bucketAxisLabel(kind: kind, date: bucket.date) : "")
                            ...
                            .monospacedDigit()
                            .foregroundColor(peak ? .monoAccent : .monoTextSecondary)
                            .frame(height: 12)
                    }
                ...
                HStack(spacing: 16) {
                    CachedAsyncImage(
                        ...
                        Circle()
                            .fill(Color.monoTextPrimary.opacity(0.07))
                            .overlay(
                                MonoIcon(icon: .profile, size: 22, color: .monoTextSecondary)
                            )
                    }
                    .aspectRatio(contentMode: .fill)
    ...
    private func reportFacts(_ report: ListeningReport) -> some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            Grid(horizontalSpacing: 18, verticalSpacing: 0) {
                ...
                Divider()
                    .gridCellColumns(2)
                    .overlay(Color.monoTextPrimary.opacity(0.08))
                    .padding(.vertical, 15)
    ...
    private func fixedTrackBar(
        ...
        return ZStack(alignment: .bottom) {
            ...
            if value > 0 {
                RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
                    .fill(isPeak ? Color.monoAccent : Color.monoTextPrimary.opacity(0.26))
    ...
    private var footer: some View {
        VStack(alignment: .leading, spacing: 5) {
            Text(String(localized: "数据仅保存在本机"))
                .font(.system(size: 11, weight: .semibold, design: .rounded))
                .foregroundColor(.monoTextSecondary)
            Text(String(localized: "Mono 听歌报告"))
    ...
    private var emptyState: some View {
        VStack(spacing: 14) {
            MonoIcon(icon: .waveform, size: 36, color: .monoTextSecondary)
