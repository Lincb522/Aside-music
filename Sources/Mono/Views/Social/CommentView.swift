...
struct CommentView: View {
    ...
    init(song: Song) {
        let resource = CommentResource(song: song)
            ?? CommentResource(source: song.musicSource, platformID: "")
        _vm = StateObject(wrappedValue: CommentViewModel(resource: resource))
        songName = song.name
            ...
            HStack(alignment: .center, spacing: 14) {
                ...
                Button { dismissCurrentPresentation(systemDismiss: dismiss, monoSheetDismiss: monoSheetDismiss) } label: {
                    ZStack {
                        ...
                        MonoIcon(icon: .close, size: 14, color: commentSecondaryText)
                    }
                }
                    ...
                    if !vm.hotComments.isEmpty {
                        ...
                        ForEach(Array(vm.hotComments.enumerated()), id: \.element.id) { index, comment in
                            CommentRow(
                                ...
                                isHot: true,
                                hotRank: index + 1,
                                allowsInteraction: vm.supportsWriting,
        ...
        Button {
            ...
            HStack(spacing: 8) {
                if vm.isLoadingMore {
                    ...
                    MonoIcon(icon: .chevronDown, size: 10, color: commentSecondaryText, lineWidth: 1.8)
                }
            }
            .foregroundColor(commentSecondaryText)
            ...
            if let reply = vm.replyTarget {
                HStack(spacing: 8) {
                    ...
                    Button {
                        ...
                        MonoIcon(icon: .xmark, size: 10, color: commentSecondaryText)
                            .padding(6)
    ...
    private var commentSeparator: Color {
        ...
        return NeumorphicStyle.isActive ? NeumorphicStyle.separator.opacity(0.5) : Color.monoSeparator
    }

    private var commentControlFill: Color {
        if SequoiaStyle.isActive { return SequoiaStyle.materialPressed.opacity(0.78) }
        return NeumorphicStyle.isActive ? NeumorphicStyle.surfacePressed : Color.monoTextPrimary.opacity(0.06)
    }

    private var commentInputFill: Color {
        if SequoiaStyle.isActive { return SequoiaStyle.materialList.opacity(0.74) }
        return NeumorphicStyle.isActive ? NeumorphicStyle.surfacePressed : Color.monoTextPrimary.opacity(0.05)
    }

    private var commentAccentFill: Color {
        if SequoiaStyle.isActive { return SequoiaStyle.accent }
        return NeumorphicStyle.isActive ? NeumorphicStyle.accent : Color.monoIconBackground
    }
    ...
    private func sortPillBackground(isSelected: Bool) -> some View {
        if SequoiaStyle.isActive {
            ...
            Capsule()
                .fill(isSelected ? Color.monoIconBackground : Color.monoTextPrimary.opacity(0.05))
        }
    }
...
struct CommentRow: View {
    ...
    private var quoteBar: Color {
        ...
        return .monoTextPrimary.opacity(0.14)
    }
                ...
                HStack(alignment: .firstTextBaseline, spacing: 6) {
                    if let rank = hotRank {
                        Text(String(format: "%02d", rank))
                            ...
                            .foregroundColor(.monoOrange.opacity(0.9))
                    }
                ...
                if allowsInteraction {
                    ...
                    // 回复
                        Button(action: onReply) {
                            MonoIcon(icon: .comment, size: 14, color: mutedText.opacity(0.78))
                                .padding(.vertical, 2)
                        }
                        .padding(.trailing, 18)
                        ...
                        Button(action: onLike) {
                            HStack(spacing: 4) {
                                MonoIcon(
                                    ...
                                    size: 14,
                                    color: comment.liked ? .monoAccentRed : mutedText.opacity(0.78)
                                )
                                if comment.likedCount > 0 {
                                    Text(formatCount(comment.likedCount))
                                        .font(.system(size: 11.5, weight: .medium, design: .monospaced))
                                        .foregroundColor(comment.liked ? .monoAccentRed : mutedText)
                                }
                            }
                            .padding(.vertical, 2)
                        }
