...
struct PodcastEpisodeBar: View {
    ...
    private var dotColor: Color {
        isAside ? .monoAccent : .monoAccentRed
    }
    ...
    var body: some View {
        VStack(spacing: 8) {
                ...
                ZStack(alignment: .leading) {
                    HStack(spacing: 0) {
                        ForEach(0..<40, id: \.self) { i in
                            let isMajor = i % 5 == 0
                            Rectangle()
                                .fill(Color.monoTextSecondary.opacity(isAside ? (isMajor ? 0.32 : 0.12) : (isMajor ? 0.4 : 0.15)))
            ...
            Text(String(format: String(localized: "podcast_episode_indicator"), currentEpisodeNumber, totalCount))
                ...
                .foregroundColor(.monoTextSecondary.opacity(isAside ? 0.85 : 1))
        }
    }
}
