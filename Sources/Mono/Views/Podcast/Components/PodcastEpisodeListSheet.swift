...
struct PodcastEpisodeListSheet: View {
            ...
            if viewModel.programs.isEmpty {
                ...
                VStack(spacing: 12) {
                    if MinimalWhiteStyle.isActive {
                        ...
                        MonoIcon(icon: .micSlash, size: 44, color: secondaryTextColor.opacity(0.38))
                    }
                ...
                VStack(spacing: 12) {
                    if MinimalWhiteStyle.isActive {
                        ...
                        MonoIcon(icon: .search, size: 40, color: secondaryTextColor.opacity(0.38))
                    }
    ...
    private var headerSection: some View {
        VStack(spacing: 14) {
            HStack(spacing: 14) {
                ...
                HStack(spacing: 8) {
                    Button(action: { viewModel.toggleEpisodeOrder() }) {
                        HStack(spacing: 5) {
                            MonoIcon(icon: .filter, size: 11, color: controlColor)
            ...
            if isSearchExpanded {
                HStack(spacing: 10) {
                    MonoIcon(icon: .search, size: 14, color: controlColor)
                    ...
                    Button {
                        ...
                        MonoIcon(icon: .close, size: 12, color: controlColor)
        ...
        return HStack(spacing: 14) {
            ZStack {
                if isCurrent && viewModel.isRadioPlaying {
                    ...
                    MonoIcon(icon: .pause, size: 14, color: activeTint, lineWidth: 1.6)
                } else {
                    Text("\(episodeNumber)")
    ...
    private func podcastControlBackground(cornerRadius: CGFloat) -> some View {
        if NeumorphicStyle.isActive {
            ...
            Color.monoSeparator.opacity(0.9)
        }
    }
    ...
    private var coverPlaceholderFill: Color {
        ...
        return SequoiaStyle.isActive ? SequoiaStyle.materialList : Color.monoTextSecondary.opacity(0.08)
    }
