...
private enum PodcastQueuePalette {
    ...
    static var separator: Color {
        ...
        return NeumorphicStyle.isActive ? NeumorphicStyle.separator.opacity(0.42) : .monoSeparator
    }
...
struct PodcastPlaylistPopupView: View {
    ...
    private var headerView: some View {
        HStack(spacing: 0) {
            ...
            HStack(spacing: 8) {
                ...
                // 播放模式
                HStack(spacing: 6) {
                    MonoIcon(icon: .repeatMode, size: 16, color: PodcastQueuePalette.secondaryText)
    ...
    private var searchFieldView: some View {
        HStack(spacing: 10) {
            MonoIcon(icon: .search, size: 14, color: PodcastQueuePalette.secondaryText)
            ...
            Button {
                ...
                MonoIcon(icon: .close, size: 12, color: PodcastQueuePalette.secondaryText)
    ...
    private var emptyView: some View {
        VStack(spacing: 16) {
            ...
            if MinimalWhiteStyle.isActive {
                ...
                MonoIcon(icon: .radio, size: 48, color: PodcastQueuePalette.mutedText.opacity(0.36))
            }
...
private struct PodcastQueueRow: View {
    ...
    private var leadingIndicator: some View {
        ...
        case .played:
            MonoIcon(icon: .history, size: 12, color: PodcastQueuePalette.mutedText.opacity(0.62), lineWidth: 1.5)
                .frame(width: 18, height: 18)
        case .upcoming:
            MonoIcon(icon: .play, size: 12, color: PodcastQueuePalette.mutedText.opacity(0.42), lineWidth: 1.5)
                .frame(width: 18, height: 18)
        }
    }
        ...
        HStack(spacing: 10) {
            ...
            if let removeAction, !isCurrent {
                Button(action: removeAction) {
                    MonoIcon(icon: .xmark, size: 11, color: PodcastQueuePalette.mutedText.opacity(0.55), lineWidth: 1.6)
