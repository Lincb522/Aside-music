...
class CommentViewModel: ObservableObject {
    ...
    var resourceId: Int { resource.numericID ?? 0 }
    var resourceType: CommentType {
        CommentType(rawValue: resource.type.rawValue) ?? .song
    }
    var source: MusicSource { resource.source }
    var supportsWriting: Bool { resource.supportsWriting }
    var supportsSorting: Bool { resource.supportsSorting }
    ...
    init(resource: CommentResource) {
        self.resource = resource
    }

    convenience init(resourceId: Int, resourceType: CommentType = .song) {
        self.init(
            resource: CommentResource(
                source: .netease,
                type: CommentResourceType(rawValue: resourceType.rawValue) ?? .song,
                platformID: String(resourceId)
            )
        )
    }
    ...
    func loadComments() {
        ...
        // 同时加载热评和普通评论
        let hotPublisher = api.fetchPlatformHotComments(resource: resource, limit: 5)
            .replaceError(with: [])
            .setFailureType(to: Error.self)
        let commentsPublisher = api.fetchPlatformComments(
            resource: resource,
            page: 1,
            pageSize: pageSize,
            sortType: sortType
        ...
        Publishers.Zip(hotPublisher, commentsPublisher)
            ...
            .sink { [weak self] completion in
                ...
                self.totalCount = commentData.totalCount
                self.hasMore = commentData.hasMore
    ...
    func toggleLike(comment: Comment, isHot: Bool) {
        guard supportsWriting else { return }
        ...
        publisher
            ...
            .sink { [weak self] completion in
                ...
                if response.code == 200 {
                    ...
                    self.errorMessage = response.message ?? String(localized: "发送失败")
                }
            }
            .store(in: &cancellables)
    }
