...
final class CustomFontManager: ObservableObject {
    ...
    /// 在线字体导入使用的分阶段入口，让下载页可以显示解压、校验与注册进度。
    @discardableResult
    func importFonts(
        from sourceURLs: [URL],
        progress: @MainActor (Double) -> Void
    ) async throws -> [ImportedFontRecord] {
        progress(0.04)
        await Task.yield()
        ...
        var fontURLs: [URL] = []
        let sourceCount = max(1, sourceURLs.count)
        for (index, sourceURL) in sourceURLs.enumerated() {
            ...
            let fraction = Double(index + 1) / Double(sourceCount)
            progress(0.08 + fraction * 0.34)
            await Task.yield()
        }

        progress(0.56)
        await Task.yield()
        let imported = try importFontFiles(from: fontURLs)
        progress(0.94)
        await Task.yield()
        progress(1)
        return imported
    ...
    private func importFontFiles(from sourceURLs: [URL]) throws -> [ImportedFontRecord] {
        var resolvedRecords: [ImportedFontRecord] = []
        ...
        for sourceURL in sourceURLs {
            ...
            let existingRecords = discoveredRecords.filter { candidate in
                fonts.contains(where: { $0.id == candidate.id })
            }
            ...
            resolvedRecords.append(contentsOf: existingRecords)
            resolvedRecords.append(contentsOf: newRecords)
        }
        ...
        persist()
        var seen = Set<String>()
        return resolvedRecords.filter { seen.insert($0.id).inserted }
    }
    ...
    private func extractFontFiles(from archiveURL: URL) throws -> (
        ...
        let extractionDirectory = fileManager.temporaryDirectory
            .appendingPathComponent("MonoFontImport-\(UUID().uuidString)", isDirectory: true)
        ...
        do {
            ...
            for entry in archive where entry.type == .file {
                ...
                guard extractedURLs.count < maximumArchiveFontCount,
                      totalUncompressedBytes <= maximumArchiveUncompressedBytes,
            ...
            AppLogger.error(
                "[CustomFont] ZIP import failed file=\(archiveURL.lastPathComponent) error=\(error.localizedDescription)",
                step: "font.archive"
            )
            ...
            AppLogger.error(
                "[CustomFont] ZIP extraction failed file=\(archiveURL.lastPathComponent) error=\(error.localizedDescription)",
                step: "font.archive"
            )
            try? fileManager.removeItem(at: extractionDirectory)
            throw CustomFontImportError.invalidArchive
...
private struct MonoPlayerDisplayFontModifier: ViewModifier {
    let size: CGFloat
...
extension View {
    func monoPlayerDisplayFont(
        size: CGFloat,
        weight: Font.Weight = .semibold,
        fallback: Font
    ) -> some View {
        modifier(
            MonoPlayerDisplayFontModifier(
                size: size,
                weight: weight,
                fallback: fallback
            )
...
extension String {
    ...
    func monoUppercasingEnglish() -> String {
        var result = ""
