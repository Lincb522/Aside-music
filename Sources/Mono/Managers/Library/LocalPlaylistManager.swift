...
class LocalPlaylistManager: ObservableObject {
    ...
    /// 将云端可同步歌单合并到本地，并保留基线时间之后产生的本地修改。
    ///
    /// 本地音乐歌单与历史下载歌单不参与删除；远端缺失的“我喜欢”只清空内容，
    /// 其余未修改的普通歌单按远端状态删除。返回合并后的可同步歌单数量。
    @discardableResult
    ...
    static func parseExportFile(url: URL) throws -> (name: String, songIds: [Int]) {
        ...
        let name = json["name"] as? String ?? String(localized: "lib_import_playlist")
