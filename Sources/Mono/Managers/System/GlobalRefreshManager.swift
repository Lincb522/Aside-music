...
class GlobalRefreshManager: ObservableObject {
    ...
    private var appLaunchPreloadTask: Task<Void, Never>?
    ...
    /// App 启动第一步：无条件预加载本机数据，不读取 Token 或登录状态。
    func triggerAppLaunchPreload() {
        guard appLaunchPreloadTask == nil else {
            AppLogger.debug("App 启动本地预加载已在进行，忽略重复触发")
            return
        }
        AppLogger.debug("App 启动，开始无条件本地预加载...")
        // PassthroughSubject 不缓存事件；先创建首页 VM，避免启动刷新在首页订阅前丢失。
        _ = HomeViewModel.shared
        
        appLaunchPreloadTask = Task { @MainActor [weak self] in
            guard let self else { return }
            defer { self.appLaunchPreloadTask = nil }
            isPreloading = true
            loadingMessage = String(localized: "正在加载缓存数据...")
            loadingProgress = 0.1
            
            // 1. 快速预加载 - 立即显示缓存数据
            await OptimizedCacheManager.shared.quickPreload()
            loadingProgress = 0.3
            loadingMessage = String(localized: "正在加载本地缓存...")
            loadingProgress = 0.6
    ...
    /// App 启动在线刷新：与本地预加载彻底拆开，只在账号条件满足后调用。
    func triggerAuthenticatedAppLaunchRefresh() {
        let apiService = APIService.shared
        let isLoggedIn = UserDefaults.standard.bool(forKey: AppConfig.StorageKeys.isLoggedIn)
            || apiService.isLoggedIn
            || apiService.currentCookie != nil
        guard isLoggedIn, OnlineAccessManager.shared.hasStoredToken else {
            AppLogger.debug("登录态或 Token 尚未就绪，跳过本次在线启动刷新")
            return
        }
    ...
    func triggerLoginRefresh() {
        ...
        Task { @MainActor in
            isPreloading = true
            loadingMessage = String(localized: "正在加载您的数据...")
            loadingProgress = 0.1
