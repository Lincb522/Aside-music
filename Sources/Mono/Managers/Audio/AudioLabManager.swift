// [DEPRECATED] 音频实验室管理器 - 智能分析功能已废弃，代码保留仅供参考
// 功能实现分布在以下扩展文件中（均已废弃）：
...
class AudioLabManager: ObservableObject {
    ...
    @Published var isSmartEffectsEnabled: Bool = false {
        didSet {
            guard isSmartEffectsEnabled != oldValue else { return }
            guard !isSmartEffectsEnabled else {
                AppLogger.debug("智能音效功能已废弃，忽略启用请求")
                isSmartEffectsEnabled = false
                return
            }
            saveState()
            resetToManualMode()
    ...
    enum AnalysisError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .invalidUrl: return String(localized: "无效的歌曲 URL")
            case .urlNotAvailable: return String(localized: "无法获取歌曲播放地址")
            case .downloadFailed: return String(localized: "下载音频文件失败")
            }
        }
    }
    ...
    private func setupObservers() {
        // 监听歌曲切换，先清除上一首的智能参数，避免下一首沿用旧效果
        PlayerManager.shared.$currentSong
            .dropFirst()
            .sink { [weak self] song in
                guard let self = self, self.isSmartEffectsEnabled else { return }
    ...
    func saveState() {
        UserDefaults.standard.removeObject(forKey: AppConfig.StorageKeys.audioLabSmartEffects)
