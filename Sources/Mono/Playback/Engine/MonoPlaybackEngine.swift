...
class PlayerManager: ObservableObject {
    ...
    private(set) lazy var continuity = MonoContinuityEngine(player: self)
    ...
    /// audible. API metadata is only a fallback when the source omits duration.
    ...
    /// KCM 当前音质，与 NCM 的运行时音质状态隔离。
    @Published var kugouSoundQuality: SoundQuality = {
        PlayerManager.initialKugouPlaybackQuality()
    }()
    ...
    /// 最近实际播放过的输入。上一曲优先复用，避免再次取址或重复下载。
    var recentPlaybackInputs: [String: RecentPlaybackInput] = [:]
    var preresolvedHistoryInput: RecentPlaybackInput?
    ...
    /// 当前 KCM 歌曲的音质是否由用户手动改过
    var hasManualKugouQualityOverride = false
    ...
    /// 每个播放会话只允许结算一次 EOF；拦截迟到的 stopped/error 双回调。
    var lastHandledPlaybackFinishSessionId: Int?
    ...
    var qualityButtonText: String {
        if currentSong?.isAppleMusic == true {
            return "Apple Music"
        }
        ...
        if currentSong?.isKugou == true {
            return kugouSoundQuality.buttonText
        }
    ...
    init() {
        ...
        #endif
        // 恢复变调设置
        let savedPitch = UserDefaults.standard.float(forKey: "mono_pitch_semitones")
        if savedPitch != 0 {
...
class StreamPlayerDelegateAdapter: StreamPlayerDelegate, @unchecked Sendable {
        ...
        Task { @MainActor [weak self] in
            ...
            case .stopped:
                ...
                if let continuityTarget = pm.continuity.activeTransitionRecoveryTarget(
                    ...
                    pm.mediaResolver.cancelLoadWatchdog()
                    guard pm.networkDisconnectRetryCount < pm.maxNetworkDisconnectRetries else {
                        pm.networkDisconnectRetryCount = 0
                        pm.cancelGaplessPreparation(resetPendingState: true)
                        pm.showPlaybackError(
                            song: continuityTarget,
                            error: APIService.PlaybackError.networkError
                        )
                        return
                    }
                    pm.networkDisconnectRetryCount += 1
                    PlaybackURLCache.shared.invalidate(song: continuityTarget)
                    AppLogger.warning(
                        "[MonoContinuity] 预装目标提前停止，恢复真实下一首 target=\(continuityTarget.name)",
                        step: "playback.continuity.target-stopped"
                    )
                    pm.loadAndPlay(
                        song: continuityTarget,
                        fadeInDuration: 0.5,
                        fadeInReason: "continuity target stopped",
                        preserveRetryBudget: true
                    )
                    return
                }
            ...
            case .error:
                ...
                if let continuityTarget = pm.continuity.activeTransitionRecoveryTarget(
                    ...
                    PlaybackURLCache.shared.invalidate(song: continuityTarget)
                    AppLogger.warning(
                        "[MonoContinuity] 预装目标断流，刷新真实下一首而非重播上一首 target=\(continuityTarget.name)",
                        step: "playback.continuity.target-error"
                    )
                    pm.loadAndPlay(
                        song: continuityTarget,
                        fadeInDuration: 0.5,
                        fadeInReason: "continuity target recovery",
                        preserveRetryBudget: true
                    )
                    return
                }
    ...
    /// 处理底层播放器的管线切换回调，并按当前会话落地音质切换、循环或无缝下一首。
    ///
    /// 回调携带的会话代际必须仍然有效，防止迟到事件覆盖用户刚发起的切歌或停止操作。
    func playerDidTransitionToNextTrack(_ player: StreamPlayer) {
        ...
        Task { @MainActor [weak self] in
            ...
            if let seekTime = pm.pendingQualitySwitchSeek {
                ...
                pm.isSeeking = false
            } else if pm.continuity.pendingLoopRestart {
                AppLogger.info("单曲循环无缝回绕")
                pm.continuity.pendingLoopRestart = false
