...
extension PlayerManager {
    ...
    func next() {
        ...
        if isGaplessPlaybackEnabled,
           ...
           !continuity.pendingLoopRestart,
           continuity.gaplessArmedSessionId == playbackSessionId,
           ...
           streamPlayer.isNextTrackReady {
            AppLogger.info("[MonoContinuity] 手动切歌命中已装配管线: \(nextSong.name)")
    ...
    /// 切换 KCM 音质，不修改 NCM 的运行时音质状态。
    func switchKugouQuality(_ quality: SoundQuality) {
        guard isCurrentPlaybackQualitySelectable else { return }
        guard kugouSoundQuality != quality || !hasManualKugouQualityOverride else { return }
        guard let current = currentSong, current.isKugou else {
            kugouSoundQuality = quality
            return
        }

        kugouSoundQuality = quality
        hasManualKugouQualityOverride = true
        restartCurrentSongForQualityChange(song: current, startTime: currentTime)
    }
    ...
    private func handleQualitySwitchTimeout(startTime: Double) {
        ...
        if current.isQQMusic {
            ...
            hasManualQQQualityOverride = true
        } else if current.isKugou {
            if let nextLower = SoundQuality.nextLower(than: kugouSoundQuality) {
                kugouSoundQuality = nextLower
                AppLogger.warning("KCM 音质切换超时，自动降级到 \(nextLower.displayName)")
            } else {
                AppLogger.warning("KCM 音质切换超时，保持当前音质重连")
            }
            hasManualKugouQualityOverride = true
        } else {
