...
extension PlayerManager {
    ...
    func playerDidFinishPlaying() {
        ...
        let finishingSessionId = playbackSessionId
        guard lastHandledPlaybackFinishSessionId != finishingSessionId else {
            AppLogger.warning(
                "[MonoContinuity] 同一会话收到重复播放完成回调，忽略 session=\(finishingSessionId)",
                step: "playback.completion.duplicate"
            )
            return
        }
        lastHandledPlaybackFinishSessionId = finishingSessionId
    ...
    func applyResolvedPlaybackQuality(_ resolved: ResolvedPlaybackQuality?, for song: Song) {
        ...
        switch resolved {
        case .netease(let songId, let quality)
            where !song.isQQMusic && !song.isQishui && !song.isKugou && song.id == songId:
            soundQuality = quality
        case .kugou(let hash, let quality)
            where song.isKugou && (song.kugouHash ?? String(song.id)) == hash:
    ...
    func completeManualPreparedSwitch(sessionId: Int, engineInput: String?) {
        ...
        disarmContinuityEngine()
        continuity.scheduledGaplessPreparationSessionId = nil
