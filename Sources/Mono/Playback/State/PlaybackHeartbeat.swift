        ...
        if player.appleMusicPlayback.tick() {
            ...
            player.repairSystemPlaybackSurfacesIfNeeded(
                reason: player.isAppInBackground
                    ? "background Apple Music heartbeat"
                    : "foreground Apple Music heartbeat"
            )

            let lyricIdx = LyricViewModel.shared.currentLineIndex
            let lyricChanged = lyricIdx
                != player.nowPlayingController.lastNowPlayingLyricIndex
