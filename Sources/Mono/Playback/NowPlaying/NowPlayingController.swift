...
final class NowPlayingController {
    ...
    private var reportedPlaybackRate: Double {
        player.currentSong?.isAppleMusic == true
            ? 1
            : Double(player.playbackSpeed)
    }

    private func resolvedArtworkURL(for song: Song) -> URL? {
        if song.isAppleMusic {
            // MusicKit 当前队列里的封面 URL 比持久化歌曲模型中的 URL 更新，
            // 可避免旧地址过期或资料库歌曲最初没有带 artwork。
            return player.appleMusicPlayback.activeArtworkURL ?? song.coverUrl
        }
    ...
    func repairSystemPlaybackSurfacesIfNeeded(reason: String) {
        ...
        let expectsArtwork = resolvedArtworkURL(for: song) != nil
        let hasArtwork = info?[MPMediaItemPropertyArtwork] is MPMediaItemArtwork
    ...
    func updateNowPlayingInfo() {
        ...
        // ApplicationMusicPlayer 会自行发布一部分系统媒体字段。AM 播放时在
        // 现有字典上补齐 Mono 的歌词、队列和身份，避免把系统已提供的封面抹掉。
        var info = currentSong?.isAppleMusic == true
        ...
        if let currentSong {
            ...
            if cachedArtworkSongId == PlayerManager.playbackIdentityKey(for: currentSong),
               let cachedArtworkImage {
                info[MPMediaItemPropertyArtwork] = cachedArtworkImage
            }
        }
        ...
        info[MPNowPlayingInfoPropertyPlaybackRate] = player.isPlaying ? reportedPlaybackRate : 0.0
        info[MPNowPlayingInfoPropertyDefaultPlaybackRate] = reportedPlaybackRate
    ...
    func updateNowPlayingTime() {
        ...
        if let song = player.currentSong, lyricVM.hasCurrentLyrics(for: song) {
    ...
    func updateNowPlayingArtwork(for song: Song?) {
        guard let song, let coverUrl = resolvedArtworkURL(for: song) else { return }
        let songId = song.id
        ...
        Task.detached { [weak self] in
            do {
                ...
                if let containerURL = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: groupID) {
                    ...
                    defaults?.set(identityKey, forKey: "widget_cover_identity")
                }
