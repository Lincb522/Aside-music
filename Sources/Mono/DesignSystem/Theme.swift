...
extension Color {
    ...
    static let monoBlue = Color(hex: "007AFF")
    static let monoBlueLight = Color(hex: "007AFF").opacity(0.1)
    static let monoOrange = Color(hex: "FF9500")
    static let monoOrangeLight = Color(hex: "FF9500").opacity(0.1)
    
    static var monoGradientTop: Color {
        Color(light: Color(hex: "F5F5F7"), dark: Color(hex: "1C1C1E"))
    }
    static var monoGradientBottom: Color {
        Color(light: Color(hex: "E8E8ED"), dark: Color(hex: "000000"))
    }

    private static var monoDefaultAccent: Color {
    ...
    @available(*, deprecated, message: "使用 .glassEffect() 替代")
    static var monoGlassOverlay: Color {
        if MinimalWhiteStyle.isActive { return MinimalWhiteStyle.glassFill }
    ...
    /// Sheet 面板背景叠加色
    @available(*, deprecated, message: "使用 .glassEffect() 替代")
    static var monoSheetOverlay: Color {
        if MinimalWhiteStyle.isActive { return MinimalWhiteStyle.glassStrongFill }
    ...
    /// 新通用 Sheet 面板顶部高光
    static var monoSheetHighlight: Color {
    ...
    /// 新通用 Sheet 阴影色
    static var monoSheetShadow: Color {
    ...
    /// 新通用 Sheet 顶部拖拽把手
    static var monoSheetHandle: Color {
...
struct MonoGlassCardBackground: View {
...
extension View {
    /// 给视图添加毛玻璃卡片背景（替代纯色 monoCardBackground）
    func monoGlassCard(cornerRadius: CGFloat = 20) -> some View {
        self.background(
