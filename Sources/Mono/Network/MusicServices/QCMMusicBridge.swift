// qcm数据模型转换
// 将 QQMusicKit 的 JSON 响应转换为 Mono 的统一模型
...
extension APIService {
    ...
    static func firstNonEmptyQQValue(in json: JSON, keys: [String]) -> String? {
        for key in keys {
            guard let rawValue = json[key]?.stringValue else { continue }
    ...
    static func qqMVArtworkURLString(_ raw: String?, vid: String) -> String? {
        if let normalized = normalizedQQArtworkURLString(raw), !normalized.isEmpty {
            return normalized.replacingOccurrences(of: "&amp;", with: "&")
        }
        guard !vid.isEmpty else { return nil }
        return "https://shp.qpic.cn/qqvideo_ori/0/\(vid)_496_280/0"
    }
...
extension APIService {
    ...
    static func convertQQSearchMV(_ json: JSON) -> QQMV? {
        ...
        // 封面
        let primaryCoverUrl = firstNonEmptyQQValue(
            in: json,
            keys: ["mv_pic_url", "pic_url", "picurl", "picUrl", "pic_url_16_9", "cover_url", "coverUrl"]
        )
        var fallbackCoverUrl = firstNonEmptyQQValue(
            in: json,
            keys: ["pic", "cover", "cover_pic"]
        )
        if fallbackCoverUrl == nil, let nestedMV = json["mv"] {
            fallbackCoverUrl = firstNonEmptyQQValue(
                in: nestedMV,
                keys: ["picurl", "cover"]
            )
        }
        let rawCoverUrl: String? = primaryCoverUrl ?? fallbackCoverUrl
        let coverUrl = qqMVArtworkURLString(rawCoverUrl, vid: vid)
    ...
    static func convertQQDetailMV(_ json: JSON) -> QQMV? {
        ...
        let rawCoverUrl = firstNonEmptyQQValue(
            in: json,
            keys: [
                "cover_pic", "picurl", "picUrl", "pic_url_16_9",
                "cover_url", "coverUrl", "pic", "cover",
            ]
        )
        let coverUrl = qqMVArtworkURLString(rawCoverUrl, vid: vid)
...
extension APIService {
    ...
    /// 将 Mono 的 SoundQuality 映射到 qcm的 SongFileType
