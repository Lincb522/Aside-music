...
extension APIService {
    ...
    func fetchDJPrograms(radioId: Int, limit: Int = 30, offset: Int = 0, asc: Bool = false) -> AnyPublisher<[RadioProgram], Error> {
        ncm.fetch([RadioProgram].self, keyPath: "programs") { [ncm] in
            try await ncm.djProgram(rid: radioId, limit: limit, offset: offset, asc: asc)
        }
    }
