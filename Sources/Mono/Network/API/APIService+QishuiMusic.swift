...
extension APIService {
    ...
    static func qishuiProxyPlayURL(trackId: Int, quality: String = "highest") -> String {
        let fallbackURL = "\(qishuiBaseURL)/play/\(trackId)"
        guard var components = URLComponents(string: fallbackURL) else { return fallbackURL }
        var queryItems = [
            URLQueryItem(name: "quality", value: quality),
            URLQueryItem(name: "sessionid_ss", value: qishuiSessionID),
        ]
        if let token = SecureConfig.apiToken?.trimmingCharacters(in: .whitespacesAndNewlines),
           !token.isEmpty {
            queryItems.append(URLQueryItem(name: "token", value: token))
            queryItems.append(URLQueryItem(name: "deviceId", value: DeviceIdentifier.uuid))
        }
        components.queryItems = queryItems
        return components.url?.absoluteString ?? fallbackURL
    }

    private func applyQishuiApplicationAuthorization(to request: inout URLRequest) {
...
extension APIService {
    ...
    private func _fetchQishuiQualities(trackId: Int) async throws -> [QishuiQualityInfo] {
        ...
        applyQishuiApplicationAuthorization(to: &request)
