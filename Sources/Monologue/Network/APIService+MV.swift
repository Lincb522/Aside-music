...
extension APIService {
    ...
    /// 获取全部 MV（支持筛选和分页）
    func fetchAllMVs(area: String = String(localized: "全部"), type: String = String(localized: "全部"), order: String = String(localized: "上升最快"), limit: Int = 12, offset: Int = 0) -> AnyPublisher<[MV], Error> {
        ncm.publisher { [ncm] in
    ...
    /// 获取最新 MV
    func fetchLatestMVs(area: String = String(localized: "全部"), limit: Int = 12) -> AnyPublisher<[MV], Error> {
        ncm.publisher { [ncm] in
            let areaEnum = MvArea(rawValue: area) ?? .all
