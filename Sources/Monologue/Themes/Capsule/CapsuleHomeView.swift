    ...
    private var commandBar: some View {
        HStack(spacing: 12) {
            ...
            CapsuleActionButton(tint: CapsuleStyle.mint, action: { showPersonalFM = true }) {
                MonologueIcon(icon: .fm, size: 17, color: CapsuleStyle.mint, lineWidth: 1.7)
            }

            CapsuleActionButton(action: { navigationPath.append(HomeView.HomeDestination.search) }) {
                MonologueIcon(icon: .magnifyingGlass, size: 17, color: CapsuleStyle.accent, lineWidth: 1.7)
            }
        }
        .padding(.horizontal, DeviceLayout.homeHorizontalPadding)
    }
    ...
    private var avatarView: some View {
        ...
        if let avatarUrl = viewModel.userProfile?.avatarUrl, let url = URL(string: avatarUrl) {
            ...
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                ...
                .frame(width: size, height: size)
                .overlay(MonologueIcon(icon: .profile, size: 18, color: CapsuleStyle.inkMuted, lineWidth: 1.7))
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
                        .stroke(CapsuleStyle.hairline, lineWidth: 1)
                )
        ...
        Button(action: action) {
            ZStack(alignment: .bottomLeading) {
                ...
                HStack(spacing: 8) {
                    ...
                    MonologueIcon(icon: .chevronRight, size: 11, color: .white.opacity(0.86), lineWidth: 1.7)
                }
    ...
    var body: some View {
        Button(action: action) {
            HStack(spacing: 12) {
                ...
                MonologueIcon(icon: .play, size: 15, color: tint, lineWidth: 1.8)
                    .frame(width: 34, height: 34)
        ...
        Button(action: action) {
            VStack(alignment: .leading, spacing: 10) {
                ZStack(alignment: .bottomTrailing) {
                    ...
                    MonologueIcon(icon: .play, size: 13, color: CapsuleStyle.onAccent, lineWidth: 1.7)
    ...
    var body: some View {
        CachedAsyncImage(url: url, width: size, height: size) {
            RoundedRectangle(cornerRadius: radius, style: .continuous)
                ...
                .overlay(MonologueIcon(icon: .musicNote, size: 18, color: tint.opacity(0.72), lineWidth: 1.7))
        }
        .aspectRatio(contentMode: .fill)
