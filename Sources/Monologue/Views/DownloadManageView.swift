...
private struct DownloadProgressGlyph: View {
    ...
    var body: some View {
        ZStack {
            ...
            MonologueIcon(icon: .download, size: 12, color: tint, lineWidth: 1.7)
...
struct CurrentSongDownloadSheet: View {
    ...
    var body: some View {
                    ...
                    VStack(spacing: 18) {
                        ...
                        if let song = player.currentSong {
                            ...
                            if let error = lyricDownloadManager.lastError, !error.isEmpty {
                                HStack(spacing: 9) {
                                    MonologueIcon(icon: .warning, size: 15, color: .monologueAccentRed)
                                    Text(error)
        ...
        .monologueSheetSurface(id: "current-song-download") {
            DownloadCenterPalette.backdrop
        }
    }
    ...
    private var sheetHeader: some View {
        HStack(spacing: 12) {
            ...
            Button(action: close) {
                MonologueIcon(icon: .close, size: 14, color: DownloadCenterPalette.secondary)
    ...
    private func downloadOptionRow(
        ...
        HStack(spacing: 13) {
            ...
            if showsProgress {
                ...
                MonologueIcon(icon: .chevronRight, size: 11, color: DownloadCenterPalette.secondary)
            }
        }
    ...
    private func artworkPlaceholder(size: CGFloat) -> some View {
        RoundedRectangle(cornerRadius: 16, style: .continuous)
            ...
            .frame(width: size, height: size)
            .overlay(MonologueIcon(icon: .musicNote, size: 21, color: DownloadCenterPalette.secondary))
    }
...
struct DownloadManageView: View {
        ...
        HStack(spacing: 13) {
            ZStack {
                ...
                if activeTaskCount == 0 {
                    MonologueIcon(icon: .download, size: 22, color: DownloadCenterPalette.accent)
                } else {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text(
                    activeTaskCount == 0
                        ...
                        : String(format: String(localized: "%d 个任务进行中"), activeTaskCount)
                )
                .font(.system(size: 11.5, weight: .medium, design: .rounded))
                .foregroundStyle(DownloadCenterPalette.secondary)
            }
    ...
    private var overviewCard: some View {
        VStack(alignment: .leading, spacing: 18) {
            HStack(alignment: .top, spacing: 16) {
                ...
                MonologueIcon(icon: .storage, size: 24, color: DownloadCenterPalette.accent)
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 8) {
                ...
                if showsClearAction {
                    ...
                    .foregroundStyle(Color.monologueAccentRed)
                    .buttonStyle(.plain)
                }
            }
    ...
    private func audioRow(_ record: DownloadedSong) -> some View {
        HStack(spacing: 13) {
            ...
            rowAction(icon: .trash, tint: .monologueAccentRed) {
            ...
            VStack(alignment: .leading, spacing: 7) {
                ...
                GeometryReader { proxy in
                    ZStack(alignment: .leading) {
                        ...
                        Capsule()
                            .fill(record.status == .failed ? Color.monologueAccentRed : DownloadCenterPalette.accent)
                ...
                Text(taskStatus(record, progress: progress))
                    ...
                    .foregroundStyle(record.status == .failed ? Color.monologueAccentRed : DownloadCenterPalette.secondary)
            }
    ...
    private func emptyState(icon: MonologueIcon.IconType, title: String) -> some View {
        VStack(spacing: 11) {
            MonologueIcon(icon: icon, size: 28, color: DownloadCenterPalette.secondary.opacity(0.56))
            Text(title)
                .font(.system(size: 13, weight: .semibold, design: .rounded))
                .foregroundStyle(DownloadCenterPalette.secondary)
        }
    ...
    private func rowAction(
        ...
        Button(action: action) {
            MonologueIcon(icon: icon, size: 14, color: tint)
                .frame(width: 34, height: 34)
    ...
    private func artworkPlaceholder(size: CGFloat) -> some View {
        RoundedRectangle(cornerRadius: 15, style: .continuous)
            .fill(DownloadCenterPalette.primary.opacity(0.06))
            .frame(width: size, height: size)
            .overlay(MonologueIcon(icon: .musicNote, size: 18, color: DownloadCenterPalette.secondary))
    }
    ...
    private func confirmClearMusic() {
        AlertManager.shared.show(
            ...
            primaryButtonTitle: String(localized: "清空"),
