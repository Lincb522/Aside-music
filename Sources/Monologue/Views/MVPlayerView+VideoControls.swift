                ...
                Button(action: { onTogglePlayback(); onScheduleControlsHide() }) {
                    ZStack {
                        Circle()
                            ...
                            .frame(width: fullscreen ? 72 : 52, height: fullscreen ? 72 : 52)
                        MonologueIcon(
                            icon: isPlaying ? .pause : .play,
                            size: fullscreen ? 30 : 22,
                            color: .white
                        )
                        ...
                        if fullscreen {
                            Button(action: onExitFullscreen) {
                                ZStack {
                                    ...
                                    MonologueIcon(icon: .shrinkScreen, size: 20, color: .white)
                                }
                            }
...
struct MVProgressBar: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            let thumbSize: CGFloat = isSeeking ? (fullscreen ? 20 : 14) : (fullscreen ? 14 : 8)
