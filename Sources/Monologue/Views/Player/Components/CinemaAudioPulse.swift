...
/// 影院沉浸播放器的实时音频引擎 — Mineradio 播放视觉管线的 Swift 移植。
///
...
/// - `updateCinemaDynamics` / `cameraDynamicsScale` / `updateCinemaTrackProfile`（整曲动态标定）
...
final class CinemaAudioPulse: ObservableObject {
    ...
    private struct CinemaTrackProfile {
        var scale = 1.0
        var target = 1.0
        var frames = 0
        var energyAvg = 0.0
    ...
    private var dynamics = CinemaDynamics()
    private var trackProfile = CinemaTrackProfile()
    ...
    /// 切歌时重置整曲标定（对应 resetCinemaTrackProfile + resetRealtimeBeatEngine）
    @MainActor
    func resetForNewTrack() {
        lock.lock()
        rtBeat = RTBeat()
        trackProfile = CinemaTrackProfile()
    ...
    private func ingest(_ frequencyData: [Float], sampleRate: Double, rms: Double) {
        ...
        updateCinemaDynamics(rawEnergy: re, rawLow: rb)
        updateCinemaTrackProfile(energy: re, low: rb, vocal: voc, melody: rm,
    ...
    // MARK: - updateCinemaDynamics / cameraDynamicsScale（4116-4150，非 DJ）

    private func updateCinemaDynamics(rawEnergy: Double, rawLow: Double) {
    ...
    // MARK: - updateCinemaTrackProfile（4191-4217，非 DJ）

    private func updateCinemaTrackProfile(
