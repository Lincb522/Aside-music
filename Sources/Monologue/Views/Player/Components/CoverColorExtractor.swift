...
extension UIImage {
    ...
    private func extractWithCoreImage() -> ExtractedColors {
        guard let cgImage = self.cgImage else {
            return ExtractedColors(dominant: .gray, secondary: .gray, isDark: true, isTopDark: true, luminance: 0.3)
        }
        ...
        // 顶部窄条（CIImage 坐标系左下角为原点，y 大 = 图片上方）
        let topStripRect = CGRect(
            x: w * 0.1, y: h * 0.8,
        ...
        let topStrip = areaAverageColor(ciImage: ciImage, rect: topStripRect, context: context)
        
        let luminance = 0.299 * dominant.r + 0.587 * dominant.g + 0.114 * dominant.b
        let isDark = luminance < 0.5
        
        let topLuminance = 0.299 * topStrip.r + 0.587 * topStrip.g + 0.114 * topStrip.b
        let isTopDark = topLuminance < 0.5
        ...
        return ExtractedColors(dominant: domColor, secondary: secColor, isDark: isDark, isTopDark: isTopDark, luminance: luminance)
    }
