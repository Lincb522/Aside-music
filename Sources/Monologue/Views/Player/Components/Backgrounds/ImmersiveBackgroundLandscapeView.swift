                ...
                VStack(alignment: .leading, spacing: compactHeight ? 10 : 14) {
                    ...
                    VStack(spacing: 7) {
                        ...
                        Button { showMoeWalls = true } label: {
                            HStack(spacing: 9) {
                                MonologueIcon(icon: .search, size: 14, color: palette.accent)
                                Text(String(localized: "immersive_bg_moewalls"))
                                    .font(.system(size: 12, weight: .semibold, design: .rounded))
                                    .foregroundStyle(.white.opacity(0.9))
                                Spacer(minLength: 0)
                                MonologueIcon(icon: .chevronRight, size: 10, color: .white.opacity(0.34))
                            }
                    ...
                    if let importError {
                        HStack(spacing: 7) {
                            MonologueIcon(icon: .warning, size: 12, color: .monologueAccentRed)
                            Text(importError)
        ...
        if let id = currentBoundVideoID, let video = backgrounds.video(withId: id) {
            HStack(spacing: 9) {
                ...
                Button {
                    ...
                    MonologueIcon(icon: .close, size: 10, color: .monologueAccentRed)
                        .frame(width: 30, height: 30)
                        .background(Circle().fill(Color.monologueAccentRed.opacity(0.12)))
                }
                .buttonStyle(MonologueBouncingButtonStyle())
        ...
        landscapePanel {
            VStack(alignment: .leading, spacing: 10) {
                ...
                if backgrounds.library.isEmpty {
                    VStack(spacing: 10) {
                        MonologueIcon(icon: .mv, size: 27, color: .white.opacity(0.24))
    ...
    private func landscapeLibraryCard(_ video: ImmersiveVideo) -> some View {
        ...
        return Button {
            ...
            .overlay(alignment: .topLeading) {
                if selected {
                    MonologueIcon(icon: .checkmark, size: 10, color: .white)
            ...
            .overlay(alignment: .topTrailing) {
                Button {
                    ...
                    MonologueIcon(icon: .trash, size: 10, color: .white.opacity(0.9))
...
struct MoeWallsLandscapeBrowserView: View {
    ...
    private var browserHeader: some View {
        HStack(spacing: 10) {
            ...
            HStack(spacing: 8) {
                MonologueIcon(icon: .search, size: 14, color: .white.opacity(0.44))
                ...
                Button {
                    ...
                    MonologueIcon(icon: .close, size: 10, color: .white.opacity(0.65))
                        .frame(width: 27, height: 27)
                }
                .buttonStyle(.plain)
                ...
                Button { submitSearch() } label: {
                    Group {
                        if model.isSearching {
                            ...
                            MonologueIcon(icon: .search, size: 13, color: .white)
                        }
                    }
            ...
            Button { model.retryCurrentList() } label: {
                MonologueIcon(icon: .refresh, size: 15, color: .white.opacity(0.82))
                    .frame(width: 40, height: 40)
            }
            .buttonStyle(MonologueBouncingButtonStyle())
                    ...
                    ZStack(alignment: .bottomLeading) {
                        ...
                        if model.detailErrorMessage != nil {
                            Button { model.retryDetail() } label: {
                                MonologueIcon(icon: .refresh, size: 15, color: .white)
                                    .frame(width: 40, height: 40)
                                    .background(Circle().fill(Color.black.opacity(0.5)))
                            }
                            .buttonStyle(MonologueBouncingButtonStyle())
        ...
        return HStack(spacing: 8) {
            ...
            if existing == nil, model.detail?.fourKDownloadURL != nil {
                Menu {
                    ...
                    MonologueIcon(icon: .more, size: 16, color: .white.opacity(0.9))
                        .frame(width: 40, height: 40)
                        .background(
                            RoundedRectangle(cornerRadius: 11, style: .continuous)
    ...
    private func landscapeError(_ message: String) -> some View {
        HStack(spacing: 7) {
            MonologueIcon(icon: .warning, size: 11, color: .monologueAccentRed)
            Text(message)
                .font(.system(size: 10.5, weight: .medium, design: .rounded))
    ...
    private var landscapeEmptyState: some View {
        VStack(spacing: 7) {
            MonologueIcon(icon: .search, size: 18, color: .white.opacity(0.36))
    ...
    var body: some View {
        AsyncImage(url: url, transaction: Transaction(animation: nil)) { phase in
            ...
            case .failure:
                Rectangle()
                    .fill(Color.white.opacity(0.055))
                    .overlay(MonologueIcon(icon: .mv, size: 24, color: .white.opacity(0.28)))
            @unknown default:
                Rectangle().fill(Color.white.opacity(0.055))
            }
        }
