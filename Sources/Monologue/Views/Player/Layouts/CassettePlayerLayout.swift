...
struct CassettePlayerLayout: View {
    ...
    private var progress: Double {
        ...
        return min(max((isDragging ? dragValue : timePublisher.currentTime) / timePublisher.duration, 0), 1)
    }
        ...
        GeometryReader { geo in
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    // 进度条
                    progressBar(width: geo.size.width - DeviceLayout.playerHorizontalPadding * 2)
...
extension CassettePlayerLayout {
    ...
    private func cassetteBody(width: CGFloat) -> some View {
        ...
        return TimelineView(.animation(minimumInterval: 0.033, paused: !player.isPlaying)) { timeline in
    ...
    private func drawGuideSlot(ctx: inout GraphicsContext, w: CGFloat, h: CGFloat) {
        ...
        var line = Path()
        line.move(to: CGPoint(x: first - 1, y: slotY))
        line.addLine(to: CGPoint(x: last + 1, y: slotY))
    ...
    private var topBar: some View {
        HStack {
            Button(action: { dismiss() }) {
                MonologueIcon(icon: .close, size: 22, color: textPrimary)
            ...
            Button(action: {
                ...
                MonologueIcon(icon: .more, size: 22, color: textPrimary)
                    .frame(width: 44, height: 44)
                    .background(shellColor)
                    .clipShape(Circle())
                    .overlay(Circle().stroke(strokeColor, lineWidth: 1))
    ...
    private var controls: some View {
        HStack(spacing: 0) {
            ...
            Button(action: { player.switchMode() }) {
                MonologueIcon(icon: player.mode.monologueIcon, size: 18, color: textMuted)
                    .frame(width: 44, height: 44)
            ...
            Button(action: { player.previous() }) {
                MonologueIcon(icon: .previous, size: 22, color: textPrimary)
            ...
            Button(action: { player.next() }) {
                MonologueIcon(icon: .next, size: 22, color: textPrimary)
                    .frame(width: 48, height: 48)
                    .contentShape(Rectangle())
            }
            .buttonStyle(MonologueBouncingButtonStyle())
            ...
            Button(action: { showPlaylist = true }) {
                MonologueIcon(icon: .list, size: 18, color: textMuted)
                    .frame(width: 44, height: 44)
                    .contentShape(Rectangle())
            }
            .buttonStyle(MonologueBouncingButtonStyle())
