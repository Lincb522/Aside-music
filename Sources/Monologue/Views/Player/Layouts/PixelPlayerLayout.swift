...
extension PixelPlayerLayout {
    ...
    private func pixelButton(icon: MonologueIcon.IconType, rotation: Double = 0) -> some View {
        Color.clear
            .frame(width: 40, height: 40)
            .monologueGlass(cornerRadius: 0)
            .overlay(Rectangle().stroke(fg.opacity(0.5), lineWidth: 2))
            .overlay(
                MonologueIcon(icon: icon, size: 20, color: fg)
...
extension PixelPlayerLayout {
    ...
    private var hpValue: String {
        let hp = timePublisher.duration > 0 ? Int((1 - timePublisher.currentTime / timePublisher.duration) * 99) + 1 : 99
        return String(format: "%02d", max(1, min(99, hp)))
    }
    
    /// LV 值 — 根据播放时长模拟
    private var levelValue: Int {
        min(99, max(1, Int(timePublisher.currentTime / 30) + 1))
    }
    
    /// EXP 填充格数
    private var expFilled: Int {
        guard timePublisher.duration > 0 else { return 0 }
        return Int((timePublisher.currentTime / timePublisher.duration) * 6)
    }
...
extension PixelPlayerLayout {
        ...
        VStack(spacing: 10) {
            ...
            HStack(spacing: 20) {
                Button(action: { player.previous() }) {
                    pixelControlButton {
                        MonologueIcon(icon: .previous, size: 20, color: fg)
                    }
                }
                .buttonStyle(MonologueBouncingButtonStyle())
                ...
                Button(action: { player.next() }) {
                    pixelControlButton {
                        MonologueIcon(icon: .next, size: 20, color: fg)
                    }
                }
                .buttonStyle(MonologueBouncingButtonStyle())
            }
            ...
            // 功能行
            HStack(spacing: 0) {
                funcButton(icon: player.mode.monologueIcon) { player.switchMode() }
                Spacer()
                if let song = player.currentSong {
                    LikeButton(songId: song.id, isQQMusic: song.isQQMusic, song: song, size: 16, activeColor: pixelRed, inactiveColor: fg)
    ...
    private func funcButton(icon: MonologueIcon.IconType, tint: Color? = nil, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            ZStack {
                Rectangle()
                    ...
                    .frame(width: 40, height: 36)
                    .offset(x: 1, y: 1)
                MonologueIcon(icon: icon, size: 16, color: tint ?? fg, lineWidth: 1.5)
                    .frame(width: 40, height: 36)
                    .background(fg.opacity(0.08))
