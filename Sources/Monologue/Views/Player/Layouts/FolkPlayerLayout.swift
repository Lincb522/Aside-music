    ...
    var topBar: some View {
        HStack {
            Button(action: { dismiss() }) {
                MonologueIcon(icon: .close, size: 16, color: inkDark)
            ...
            Button(action: { showMoreMenu.toggle() }) {
                MonologueIcon(icon: .more, size: 16, color: inkDark)
                    .frame(width: 44, height: 44)
            }
            .buttonStyle(MonologueBouncingButtonStyle())
        }
            ...
            HStack(spacing: 0) {
                ...
                HStack(spacing: 16) {
                    Button(action: { player.switchMode() }) {
                        MonologueIcon(icon: player.mode.monologueIcon, size: 16, color: inkFaded)
                            .frame(width: 36, height: 36)
                    }
                    .buttonStyle(MonologueBouncingButtonStyle())
                    
                    Button { showComments = true } label: {
                        MonologueIcon(icon: .comment, size: 16, color: inkFaded)
                            .frame(width: 36, height: 36)
                    }
                ...
                HStack(spacing: 12) {
                    Button(action: { player.previous() }) {
                        MonologueIcon(icon: .previous, size: 16, color: inkDark)
                            .frame(width: 44, height: 44)
                    ...
                    Button(action: { player.togglePlayPause() }) {
                        ZStack {
                            ...
                            MonologueIcon(icon: player.isPlaying ? .pause : .play, size: 22, color: paperBg)
                        }
                    }
                    .buttonStyle(MonologueBouncingButtonStyle())

                    Button(action: { player.next() }) {
                        MonologueIcon(icon: .next, size: 16, color: inkDark)
                            .frame(width: 44, height: 44)
                            .background(Circle().stroke(inkFaded.opacity(0.3), lineWidth: 1))
                    }
                    .buttonStyle(MonologueBouncingButtonStyle())
                ...
                Button(action: { showPlaylist = true }) {
                    MonologueIcon(icon: .list, size: 16, color: inkFaded)
                        .frame(width: 36, height: 36)
                }
                .buttonStyle(MonologueBouncingButtonStyle())
            }
    ...
    func folkLikeButton(song: Song) -> some View {
        ...
        return Button {
            ...
            ZStack {
                ...
                MonologueIcon(icon: isLiked ? .liked : .like, size: 16, color: isLiked ? redStamp : inkFaded)
            }
        }
        .buttonStyle(MonologueBouncingButtonStyle())
    }
