...
struct BreathingPlayerLayout: View {
    ...
    var body: some View {
            ...
            ZStack {
                ...
                if let song = player.currentSong, showLyrics {
                    LyricsView(song: song) {
                        withAnimation(MonologueAnimation.smooth) {
                            showLyrics = false
                        }
        ...
        .onChange(of: player.currentSong?.id) { _, _ in
            ...
            extractPalette()
            withAnimation(MonologueAnimation.contentAppear) {
...
extension BreathingPlayerLayout {
    ...
    private func breathingStage(in size: CGSize, safeAreaInsets: EdgeInsets) -> some View {
        ...
        .animation(MonologueAnimation.smooth, value: showGestureLegend)
        .animation(MonologueAnimation.smooth, value: feedbackTitle)
    }
...
extension BreathingPlayerLayout {
    private func flashFeedback(_ title: String, detail: String? = nil, linger: Double = 1.35) {
        ...
        Task { @MainActor in
            ...
            withAnimation(MonologueAnimation.easeOut) {
                feedbackTitle = nil
                feedbackDetail = nil
            }
        }
