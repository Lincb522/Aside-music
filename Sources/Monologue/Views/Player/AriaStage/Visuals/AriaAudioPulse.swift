...
/// 将原始 FFT 频谱转换为舞台视觉使用的频段包络、节拍事件与镜头冲击。
///
/// 音频线程负责写入频谱状态；主线程通过 `snapshot()` 推进镜头包络并读取同一帧快照。
final class AriaAudioPulse: ObservableObject {
    ...
    struct Snapshot {
        ...
        var rawEnergy: Double = 0     // 未经过 AI 编排修饰的原始能量
        var energy: Double = 0        // uEnergy
        var beatPulse: Double = 0     // uBeat
        var punch: Double = 0         // beatCam.punch
        var subBass: Double = 0       // 38-74 Hz
        var lowBody: Double = 0       // 165-420 Hz
        var presence: Double = 0      // 420-2600 Hz
        var air: Double = 0           // 1.8-9.2 kHz
        ...
        var ambience: Double = 0      // 保留给背景自身的空间氛围 0~1
    }

    /// 实时节拍命中事件（节拍触觉等外部消费者用）。
    struct BeatHit: Sendable {
    ...
    private var beatHitHandler: (@Sendable (BeatHit) -> Void)?
    ...
    private var preferredAnalysisFramesPerSecond = 30
    ...
    func stop() {
        ...
        analyzer.visualMinimumInterval = 0
        resetAll()
    }
    ...
    /// Match FFT cadence to the actual visual budget. This especially avoids
    /// 96/192 kHz sources producing far more FFT windows than the stage can draw.
    @MainActor
    func setPreferredAnalysisFramesPerSecond(_ framesPerSecond: Int) {
        preferredAnalysisFramesPerSecond = min(60, max(8, framesPerSecond))
        guard isRunning, !isSuspended else { return }
        PlayerManager.shared.spectrumAnalyzer.visualMinimumInterval =
            1.0 / Double(preferredAnalysisFramesPerSecond)
    }

    // MARK: - 外部消费者接口（节拍触觉 / 张力系统）

    /// 注册节拍命中回调；回调在独立高优队列执行，可安全驱动 CoreHaptics。
    func setBeatHitHandler(_ handler: (@Sendable (BeatHit) -> Void)?) {
        lock.lock()
    ...
    private func attachAnalyzer() {
        ...
        analyzer.visualMinimumInterval = 1.0 / Double(preferredAnalysisFramesPerSecond)
    ...
    func snapshot() -> Snapshot {
        ...
        // 三套独立舞台可能在同一显示帧的不同阶段读取频谱。使用 120Hz
        // 半帧窗口合并重复加锁与包络推进，不会让任何 60Hz 舞台少一帧数据。
        if cachedSnapshotAt > 0, now - cachedSnapshotAt < 1.0 / 120.0 {
        ...
        // 将内部包络限制到着色器和视图使用的稳定范围。
        let rawAudioEnergy = min(1, max(smoothEnergy, beatPulse * 0.30))
        let bass = min(0.90, smoothBass * 1.05 + beatPulse * 0.18)
        let mid = min(0.72, smoothMid * 1.12)
        let treble = min(0.62, smoothTreb * 1.20)
        let bandActivity = paused ? 0.0 : 1.0
        let subBass = min(1, rtBeat.subFast / max(0.045, rtBeat.subPeak * 0.70)) * bandActivity
        let lowBody = min(1, rtBeat.bodyFast / max(0.045, rtBeat.bodyPeak * 0.72)) * bandActivity
        let presence = min(1, rtBeat.vocalFast / max(0.045, rtBeat.vocalPeak * 0.72)) * bandActivity
        let air = min(1, rtBeat.snapFast / max(0.035, rtBeat.snapPeak * 0.72)) * bandActivity

        let snapshot = Snapshot(
            bass: bass, mid: mid, treble: treble,
            rawEnergy: rawAudioEnergy, energy: rawAudioEnergy,
            beatPulse: beatPulse,
            punch: beatCam.punch,
            subBass: subBass,
            lowBody: lowBody,
    ...
    private func scheduleBeatCamera(beat: RealtimeBeat, now: Double) {
        ...
        amp *= 0.78
        // 镜头冲击直接跟随当前节拍的综合能量。分幕能量已不再由音频脉冲层持有，
        // 因此这里使用实时 impact，避免遗留状态让镜头与实际音乐脱节。
        amp *= 0.52 + visualImpact * 0.96
