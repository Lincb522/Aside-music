                            ...
                            if viewModel.isQRExpired {
                                ZStack {
                                    ...
                                    Button(action: {
                                        ...
                                        MonologueIcon(icon: .refresh, size: 28, color: .white)
                                    }
                                }
                            }
                        }
...
struct PhoneLoginView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            HStack(spacing: 12) {
                MonologueIcon(icon: .profile, size: 16, color: .gray.opacity(0.6))
            ...
            .background(
                RoundedRectangle(cornerRadius: 25, style: .continuous)
                    .fill(Color.monologueGlassTint)
                    .monologueGlass(cornerRadius: 25)
            )
            .cornerRadius(25)
            .shadow(color: .black.opacity(0.03), radius: 5, x: 0, y: 2)
            
            HStack(spacing: 12) {
                MonologueIcon(icon: .lock, size: 16, color: .gray.opacity(0.6))
                ...
                Button(action: {
                    ...
                    if viewModel.isCaptchaSent {
                        ...
                        MonologueIcon(icon: .send, size: 20, color: .monologueTextPrimary.opacity(0.8))
                            .padding(8)
