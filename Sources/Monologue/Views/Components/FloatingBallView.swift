...
struct FloatingBallView: View {
    ...
    private var floatingBall: some View {
        ZStack {
            // 进度环 - 轨道更柔和
            Circle()
                .stroke(Color.monologueTextPrimary.opacity(0.08), lineWidth: 2.5)
                .frame(width: ballSize, height: ballSize)
            
            let progress = timePublisher.duration > 0 ? min(max(timePublisher.currentTime / timePublisher.duration, 0), 1) : 0
            Circle()
                .trim(from: 0, to: CGFloat(progress))
                .stroke(Color.monologueAccent.opacity(0.6), style: StrokeStyle(lineWidth: 2.5, lineCap: .round))
                .frame(width: ballSize, height: ballSize)
        ...
        ZStack {
            ...
            if let song = player.currentSong {
                ...
                Circle()
                    ...
                    .frame(width: 22, height: 22)
                    .overlay(
                        MonologueIcon(icon: .musicNote, size: 10, color: .white.opacity(0.5))
                    )
            }
            ...
            ForEach(items, id: \.tab) { item in
                Button {
                    ...
                    MonologueIcon(
                        icon: item.icon,
                        size: 18,
                        color: currentTab == item.tab ? .monologueAccent : .monologueTextSecondary
                    )
