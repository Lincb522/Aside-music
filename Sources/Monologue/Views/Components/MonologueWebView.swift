...
struct WebViewRepresentable: UIViewRepresentable {
    private static let loggerHandlerName = "monologueLogger"

    private static let loggerScript = #"""
    (() => {
        if (window.__monologueNativeLoggerInstalled) return;
        window.__monologueNativeLoggerInstalled = true;

        const bridge = window.webkit && window.webkit.messageHandlers
            ? window.webkit.messageHandlers.monologueLogger
            : null;
        if (!bridge) return;
