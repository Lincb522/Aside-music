    ...
    private func analysisResultCard(_ analysis: AudioAnalysisResult) -> some View {
        VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                HStack(spacing: 12) {
                    recommendedTag(String(localized: "低音 \(analysis.recommendedEffects.bassGain > 0 ? "+" : "")\(Int(analysis.recommendedEffects.bassGain))dB"))
                    recommendedTag(String(localized: "高音 \(analysis.recommendedEffects.trebleGain > 0 ? "+" : "")\(Int(analysis.recommendedEffects.trebleGain))dB"))
                    recommendedTag(String(localized: "环绕 \(Int(analysis.recommendedEffects.surroundLevel * 100))%"))
                }
                
                HStack(spacing: 12) {
                    recommendedTag(String(localized: "混响 \(Int(analysis.recommendedEffects.reverbLevel * 100))%"))
                    recommendedTag(String(localized: "声场 \(String(format: "%.1f", analysis.recommendedEffects.stereoWidth))x"))
                    if analysis.recommendedEffects.loudnormEnabled {
                        recommendedTag(String(localized: "响度标准化"))
                    }
                }
            }
