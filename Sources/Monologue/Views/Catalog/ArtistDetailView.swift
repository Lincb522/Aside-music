    ...
    var body: some View {
        ...
        .monologueSheet(isPresented: $showFullDescription, preset: .standard){
            ArtistBioSheet(
                viewModel: viewModel,
                artistId: artistId,
                loadsRemoteDescription: initialArtist?.source != .appleMusic
            )
        }
        .onAppear {
        ...
        .onChange(of: selectedTab) { _, newTab in
            guard initialArtist?.source != .appleMusic else { return }
...
private extension ArtistDetailView {
    ...
    var artistPlatformBadge: some View {
        if viewModel.artist?.source == .appleMusic {
            PlatformBadgeLabel(
    ...
    private var artistSongRows: some View {
        LazyVStack(spacing: (CapsuleStyle.isActive || MinimalWhiteStyle.isActive) ? 4 : 0) {
            ForEach(Array(artistSongFiltered.enumerated()), id: \.element.id) { index, song in
                SongListRow(song: song, index: index, isSelecting: isArtistSelectMode, isSelected: artistSelectedSongIds.contains(song.id), onArtistTap: initialArtist?.source == .appleMusic ? nil : { artistId in
                    selectedArtistInfo = nil
                    ...
                    showSongDetail = true
                }, onAlbumTap: initialArtist?.source == .appleMusic ? nil : { albumId in
