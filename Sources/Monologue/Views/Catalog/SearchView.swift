...
struct SearchView: View {
    ...
    private var defaultSearchHeader: some View {
            ...
            HStack(spacing: 7) {
                ...
                if isPlatformLoading {
                    ...
                    Text(L10n.format("search_result_count_format", selectedPlatformResultCount))
                        .font(.rounded(size: 12, weight: .semibold))
                        .foregroundStyle(Color.monologueTextSecondary.opacity(0.85))
                        .monospacedDigit()
                }
            }
        }
    ...
    private var capsulePlatformSwitch: some View {
        let platforms: [MusicSource] = viewModel.currentTab == .songs
            ? [.netease, .qqmusic, .qishui, .kugou, .appleMusic]
            : [.netease, .qqmusic]
    ...
    private func resultCount(for source: MusicSource, tab: SearchTab) -> Int {
        ...
        case .kugou:
            return tab == .songs ? viewModel.displayedSongCount(for: .kugou) : 0
        case .appleMusic:
            return tab == .songs ? viewModel.displayedSongCount(for: .appleMusic) : 0
        case .local:
            return 0
        }
    }
    ...
    private var platformTabBar: some View {
        if MangaStyle.isActive {
                ...
                ForEach(platforms, id: \.self) { platform in
                    ...
                    Button(action: {
                        ...
                        .background {
                            if selected {
                                Capsule()
                                    .fill(Color.monologueGlassTint.opacity(0.9))
            ...
            .padding(3)
            .background(
                Capsule().fill(Color.monologueTextPrimary.opacity(0.045))
            )
            .frame(maxWidth: .infinity, alignment: .leading)
            .padding(.horizontal, DeviceLayout.viewHorizontalPadding)
            .padding(.top, 2)
    ...
    private var isPlatformEmpty: Bool {
        ...
        case .kugou: return viewModel.kugouResults.isEmpty
        case .appleMusic: return viewModel.appleMusicResults.isEmpty
    ...
    @ViewBuilder
    private var kugouResultsContent: some View {
        if viewModel.currentTab == .songs {
    ...
    @ViewBuilder
    private var appleMusicResultsContent: some View {
        if viewModel.currentTab == .songs {
    ...
    private var defaultEmptySearchView: some View {
        ScrollView {
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        defaultSearchSectionHeader(
                            ...
                            Button {
                                ...
                                MonologueIcon(icon: .trash, size: 14, color: .monologueTextSecondary.opacity(0.75))
                                    .frame(width: 30, height: 30)
                                    .background(
                            ...
                            ForEach(viewModel.searchHistory, id: \.id) { item in
                                Button {
                                    ...
                                    Text(item.keyword)
                                        ...
                                        .foregroundColor(.monologueTextPrimary.opacity(0.88))
                                        .lineLimit(1)
                                        .padding(.horizontal, 14)
                                        .padding(.vertical, 8.5)
                                        .background(
                                            Capsule().fill(Color.monologueTextPrimary.opacity(0.055))
                                        )
                                        .overlay(
                                            Capsule().stroke(Color.monologueTextPrimary.opacity(0.06), lineWidth: 1)
                                        )
                                }
                                .buttonStyle(PlainButtonStyle())
                                .contextMenu {
                                    ...
                                    HStack(spacing: 11) {
                                        Text("\(index + 1)")
                                            ...
                                            .foregroundColor(
                                                index < 3
                                                    ? .monologueAccent
                                                    : .monologueTextSecondary.opacity(0.45)
                                            )
                                            .monospacedDigit()
                                            .lineLimit(1)
    ...
    private func defaultSearchSectionHeader<Trailing: View>(
        title: String,
    ...
    private func mangaSearchShelf<Content: View>(
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 9) {
                ...
                if let actionIcon, let action {
                    Button(action: action) {
                        MonologueIcon(icon: actionIcon, size: 13, color: MangaStyle.inkSub, lineWidth: 1.7)
                            .frame(width: 30, height: 30)
                            .contentShape(Rectangle())
                    }
                    .buttonStyle(.plain)
