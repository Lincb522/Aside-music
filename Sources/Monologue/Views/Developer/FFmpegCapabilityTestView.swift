...
struct FFmpegCapabilityTestView: View {
    ...
    private var coverPlaceholder: some View {
        ZStack {
            ...
            MonologueIcon(icon: .musicNote, size: 18, color: .white.opacity(0.3))
        }
    }
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if !model.selectedEffect.parameters.isEmpty {
                ...
                Button {
                    ...
                    HStack(spacing: 8) {
                        MonologueIcon(icon: .refresh, size: 12, color: .white.opacity(0.58))
                        Text("重置参数")
                            .font(.system(size: 11.5, weight: .semibold, design: .rounded))
    ...
    private var filterSidebar: some View {
        VStack(alignment: .leading, spacing: 0) {
            VStack(alignment: .leading, spacing: 7) {
                MonologueIcon(icon: .waveform, size: 24, color: .cyan)
