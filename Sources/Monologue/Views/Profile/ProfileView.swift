...
struct ProfileView: View {
    ...
    @State private var isRequestingAppleMusicAuthorization = false
    ...
    private var appleMusicAuthorizationText: String {
        if isRequestingAppleMusicAuthorization {
            return String(localized: "apple_music_authorizing")
        }
        return appleMusicService.isAuthorized
            ? String(localized: "apple_music_authorized")
            : String(localized: "apple_music_not_authorized")
    }

    private func requestAppleMusicAuthorization() {
        guard !isRequestingAppleMusicAuthorization else { return }
        isRequestingAppleMusicAuthorization = true
        Task { @MainActor in
            _ = await appleMusicService.requestAuthorizationIfNeeded(
                refreshesSubscription: true
            )
            isRequestingAppleMusicAuthorization = false
        }
    }
    ...
    private var asideMenuIndex: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Button {
                ...
                asideMenuRow(
                    index: 2,
                    title: String(localized: "apple_music_authorization"),
                    trailingText: appleMusicAuthorizationText
                )
            }
            .buttonStyle(MonologueBouncingButtonStyle(scale: 0.98))
            .disabled(isRequestingAppleMusicAuthorization)

            asideMenuHairline

            NavigationLink(destination: ListeningStatsView()) {
                asideMenuRow(index: 3, title: String(localized: "cloud_sync_listening_stats"))
            }
            .buttonStyle(MonologueBouncingButtonStyle(scale: 0.98))
            ...
            NavigationLink(destination: CloudDiskView()) {
                asideMenuRow(index: 4, title: NSLocalizedString("profile_cloud_disk", comment: ""))
            }
            .buttonStyle(MonologueBouncingButtonStyle(scale: 0.98))
        }
    }
