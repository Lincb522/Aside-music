...
struct MVEmbeddedCommentSection: View {
            ...
            ForEach(CommentSortType.allCases, id: \.rawValue) { type in
                Button {
                    ...
                    Text(type.title)
                        ...
                        .foregroundColor(commentVM.sortType == type ? .monologueIconForeground : .monologueTextSecondary)
                        .padding(.horizontal, 14)
                        .padding(.vertical, 7)
                        .background(
                            Capsule()
                                .fill(commentVM.sortType == type ? Color.monologueIconBackground : Color.monologueTextPrimary.opacity(0.05))
                        )
                }
                .buttonStyle(.plain)
            }
    ...
    private var emptyView: some View {
        VStack(spacing: 12) {
            MonologueIcon(icon: .comment, size: 32, color: .monologueTextSecondary.opacity(0.25))
