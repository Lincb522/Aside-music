...
final class MediaSourceResolver {
    ...
    func loadAndPlay(
        ...
        let preresolvedInput = player.preresolvedRestorationInput
        ...
        let shouldDeferPresentation = isNewSong && player.currentSong != nil
        if shouldDeferPresentation {
            ...
            player.publishPlaybackPresentation(song: song, startTime: startTime, isUnblocked: false)
        }
    ...
    private func handleNeteaseSongUrlResult(
        ...
        player.updatePlaybackPresentationResolution(
            song: song,
            sessionId: sessionId,
    ...
    func startPlayback(url: URL, autoPlay: Bool = true, startTime: Double = 0, decryptionKey: String? = nil) {
        ...
        player.gapless.cancelScheduledMediaPrefetch()
        player.disarmGaplessEngine()
