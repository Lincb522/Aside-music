// MonoPlaybackEngine+NowPlaying.swift
// Monologue
//
...
extension PlayerManager {
    ...
    func repairSystemPlaybackSurfacesIfNeeded(reason: String) {
        ...
        let hasUsableIdentity = reportedTitle == song.name
            && reportedIdentity == playbackIdentityKey(for: song)
    ...
    func updateNowPlayingInfo() {
        ...
        if GameModeManager.shared.isActive && SettingsManager.shared.gameModeSilentNowPlaying {
            if SettingsManager.shared.gameModeMinimalNowPlaying {
                ...
                if let currentSong {
                    minimal[MPNowPlayingInfoPropertyExternalContentIdentifier] = playbackIdentityKey(for: currentSong)
                }
        ...
        if let currentSong {
            info[MPNowPlayingInfoPropertyExternalContentIdentifier] = playbackIdentityKey(for: currentSong)
        }
    ...
    func updateNowPlayingArtwork(for song: Song?) {
        ...
        let songId = song.id
        let identityKey = playbackIdentityKey(for: song)
        ...
        Task.detached { [weak self] in
            do {
                ...
                await MainActor.run { [weak self] in
                    ...
                    guard let current = self.currentSong,
                          self.playbackIdentityKey(for: current) == identityKey else { return }
