...
class SearchViewModel: ObservableObject {
    ...
    func selectPlatform(_ platform: MusicSource) {
        ...
        guard platform == .appleMusic,
              currentTab == .songs,
              hasSearched,
              !requestKeyword.isEmpty,
              appleMusicResults.isEmpty,
              !isAppleMusicLoading else { return }
        appleMusicCanLoadMore = true
        appleMusicCurrentPage = 0
    ...
    func switchTab(_ tab: SearchTab) {
        ...
        if tab != .songs && (selectedPlatform == .qishui || selectedPlatform == .kugou || selectedPlatform == .appleMusic) {
    ...
    func loadMore(source: MusicSource) {
        ...
        case .kugou:
            ...
            executeKugouSearch(keyword: keyword, page: kugouCurrentPage + 1, isLoadMore: true)
        case .appleMusic:
            guard !isFetchingMoreAppleMusic && appleMusicCanLoadMore else { return }
            isFetchingMoreAppleMusic = true
            executeAppleMusicSearch(
                keyword: keyword,
                page: appleMusicCurrentPage + 1,
                isLoadMore: true
            )
        case .local:
            return
    ...
    private func executeKugouSearch(keyword: String, page: Int, isLoadMore: Bool) {
        ...
        apiService.searchKugouSongsWithTotal(keyword: keyword, page: page, pageSize: 30)
            ...
            .sink(receiveCompletion: { [weak self] completion in
                ...
                SongArtworkFallbackRegistry.shared.register(result.songs)
                if isLoadMore {
                    let existingIDs = Set(self.kugouResults.map(\.id))
                    self.kugouResults.append(contentsOf: result.songs.filter { !existingIDs.contains($0.id) })
                } else {
                    self.kugouResults = result.songs
                }
        ...
        appleMusicSearchTask = Task { @MainActor [weak self] in
            ...
            do {
                ...
                if isLoadMore {
                    ...
                    appleMusicResults.append(contentsOf: result.songs.filter {
                        !existingIDs.contains(PlayerManager.playbackIdentityKey(for: $0))
                    })
                } else {
                    appleMusicResults = result.songs
                }
