// MonoPlaybackEngine+Internal.swift
// Monologue
//
...
extension PlayerManager {
    ...
    func isPreparedGaplessPipelineFresh(for song: Song) -> Bool {
        if localPlaybackURL(for: song) != nil || cachedQMCFileURL(for: song) != nil {
            return true
        }
        if song.isQishui,
    ...
    private func armGaplessPipeline(sessionId: Int) {
        ...
        if localPlaybackURL(for: next) != nil || cachedQMCFileURL(for: next) != nil {
            gaplessArmedSessionId = sessionId
            gaplessArmAttempts += 1
    ...
    func prepareNextTrackURL() {
        ...
        if let cachedQMCFile = cachedQMCFileURL(for: song) {
    ...
    private var playbackTargetSource: PlaySource {
    ...
    func loadAndPlay(
        ...
        invalidateInFlightPlaybackWork(reason: "loadAndPlay: \(song.name)")
        ...
        if shouldDeferPresentation {
            deferPendingPlaybackQueueMutationUntilCommit()
    ...
    private func settlePlaybackLoadFailure(
        ...
        _ = discardPendingPlaybackPresentationIfNeeded(
            song: song,
            sessionId: sessionId
        )
        isPlaying = streamPlayer.state == .playing && streamPlayer.isAudioOutputRunning
        isLoading = false
        refreshPlaybackSurfaceState()
        saveState()
    }
    ...
    private func loadAndPlayNeteaseSong(song: Song, autoPlay: Bool, startTime: Double) {
        let sessionId = playbackSessionId
        playbackURLCancellable?.cancel()
        let isPodcast = playbackTargetSource.isPodcast
        let modelReportedLevel = modelReportedNeteaseQuality(for: song)?.rawValue
    ...
    func startPlayback(url: URL, autoPlay: Bool = true, startTime: Double = 0, decryptionKey: String? = nil) {
        isLoading = true
        scheduleDecryptedAudioCacheCleanup()
        ...
        guard activateAudioSessionForPlaybackChecked(reason: "loadAndPlay start") else {
            let sessionId = playbackSessionId
            let song = pendingPlaybackPresentationSong ?? currentSong
            let error = NSError(
                domain: "AudioSession",
                code: -1,
        ...
        if defersPresentation,
           ...
           streamPlayer.state == .playing {
            ...
            manualSwitchPreparationTask = Task { @MainActor [weak self] in
                guard let self else { return }
                let fallbackToColdLoad: (_ reason: String) -> Void = { [weak self] reason in
                    guard let self else { return }
                    guard self.playbackSessionId == sessionId,
                          self.manualPreparedSwitchSessionId == sessionId else { return }
                    AppLogger.warning(
                        "[ManualSwitch] \(reason)，回退独立管线 target=\(self.pendingPlaybackPresentationSong?.name ?? "unknown")",
                        step: "playback.manual-switch.cold-fallback"
                    )
                    self.streamPlayer.cancelNextPreparation()
                ...
                for _ in 0..<160 {
                    ...
                    if self.streamPlayer.isNextTrackReady {
                        ...
                        for _ in 0..<60 {
                            ...
                            guard self.playbackSessionId == sessionId,
                                  self.manualPreparedSwitchSessionId == sessionId else { return }
                            if self.streamPlayer.currentPlaybackInput == input,
                               !self.streamPlayer.isTrackTransitionNotificationDeferred {
                                self.completeManualPreparedSwitch(
                                    sessionId: sessionId,
                                    engineInput: input
                                )
                                return
    ...
    private static var lastDecryptedAudioCacheSweepAt: Date = .distantPast

    /// QMC/汽水解密产物落在 tmp，单文件可达几十上百 MB；系统只在磁盘
    /// 紧张时才回收 tmp，长时间听无损会膨胀到 GB 级。这里按 LRU 控制
    /// 总量：正在使用的输入显式保护，近期触碰过的文件靠时间窗兜底。
    func scheduleDecryptedAudioCacheCleanup(force: Bool = false) {
        let now = Date()
        guard force
                || now.timeIntervalSince(Self.lastDecryptedAudioCacheSweepAt)
                    >= DecryptedAudioCachePolicy.sweepInterval else { return }
        Self.lastDecryptedAudioCacheSweepAt = now

        var protectedNames = Set<String>()
        for input in [currentPlayingURL, pendingGaplessPlaybackInput, pendingPlaybackPresentationInput] {
        ...
        Task.detached(priority: .utility) {
            PlayerManager.sweepDecryptedAudioCaches(
    ...
    nonisolated private static func sweepDecryptedAudioCaches(
        directories: [URL],
        ...
        guard totalBytes > DecryptedAudioCachePolicy.limitBytes else { return }

        let recentCutoff = Date().addingTimeInterval(-DecryptedAudioCachePolicy.recentUseProtection)
        ...
        for entry in deletable {
            guard remainingBytes > DecryptedAudioCachePolicy.targetBytes else { break }
            do {
        ...
        if removedCount > 0 {
            AppLogger.info(
                "[PlaybackCache] 解密缓存 LRU 清理 removed=\(removedCount) freed=\(removedBytes / 1_048_576)MB before=\(totalBytes / 1_048_576)MB target=\(DecryptedAudioCachePolicy.targetBytes / 1_048_576)MB",
    ...
    nonisolated private static func touchDecryptedAudioCacheFile(at url: URL) {
    ...
    private func downloadAndPlayQishuiAudio(
        ...
        if FileManager.default.fileExists(atPath: cacheFile.path) {
            ...
            Self.touchDecryptedAudioCacheFile(at: cacheFile)
    ...
    private func qmcCacheBaseName(for song: Song) -> String {
        playbackIdentityKey(for: song)
            .replacingOccurrences(of: ":", with: "_")
            .replacingOccurrences(of: "/", with: "_")
    }

    private func qmcCacheURL(for song: Song, extension ext: String) -> URL {
        Self.qmcCacheDir.appendingPathComponent("\(qmcCacheBaseName(for: song)).\(ext)")
    }

    private func cachedQMCFileURL(for song: Song) -> URL? {
        for ext in ["flac", "ogg"] {
            let scopedURL = qmcCacheURL(for: song, extension: ext)
            if FileManager.default.fileExists(atPath: scopedURL.path) {
                Self.touchDecryptedAudioCacheFile(at: scopedURL)
                return scopedURL
            }

            let legacyURL = Self.qmcCacheDir.appendingPathComponent("\(song.id).\(ext)")
            if FileManager.default.fileExists(atPath: legacyURL.path) {
                if (try? FileManager.default.moveItem(at: legacyURL, to: scopedURL)) != nil {
                    Self.touchDecryptedAudioCacheFile(at: scopedURL)
                    return scopedURL
                }
                Self.touchDecryptedAudioCacheFile(at: legacyURL)
                return legacyURL
            }
        }
        return nil
    ...
    private func prepareDecryptedNextTrack(url: URL, ekey: String, song: Song, sessionId: Int) {
        ...
        if let cachedFile = cachedQMCFileURL(for: song) {
            qmcPrefetchTask?.cancel()
            qmcPrefetchTask = nil
        ...
        qmcPrefetchTask = Task { [weak self] in
            ...
            do {
                let ext = url.pathExtension.contains("mflac") ? "flac"
                         ...
                         : "flac"
                let cachedFile = self.qmcCacheURL(for: song, extension: ext)
                ...
                if FileManager.default.fileExists(atPath: cachedFile.path),
                   ...
                   let size = attrs[.size] as? Int, size > 1024 {
                    ...
                    Self.touchDecryptedAudioCacheFile(at: cachedFile)
    ...
    func prefetchNextQQTrack(_ nextSong: Song) {
        ...
        let cacheKey = PlaybackURLCache.qqKey(mid: mid, quality: requestedQuality?.rawValue)
        if let cached = PlaybackURLCache.shared.fresh(forKey: cacheKey),
           !cached.requiresQMCDecryption || cachedQMCFileURL(for: nextSong) != nil {
            return
        }
        let modelReportedQuality = nextSong.qqMaxQuality
        let songName = nextSong.name
        let cacheBaseName = qmcCacheBaseName(for: nextSong)
        let sessionId = playbackSessionId
        ...
        qmcPrefetchTask = Task {
            ...
            do {
                ...
                let ext = url.pathExtension.contains("mflac") ? "flac"
                         : url.pathExtension.contains("mgg") ? "ogg"
                         : "flac"
                let outFile = Self.qmcCacheDir.appendingPathComponent("\(cacheBaseName).\(ext)")
    ...
    func prefetchNextNCMTrack(_ song: Song) {
        ...
        let songId = song.id
        let cacheBaseName = qmcCacheBaseName(for: song)
        let sessionId = playbackSessionId
...
/// QMC/汽水解密缓存的容量策略。
private enum DecryptedAudioCachePolicy {
