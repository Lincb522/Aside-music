...
final class SpecialGreetingManager: ObservableObject {
    ...
    private func dailyMessage(
        ...
        if let cached, cached.dayKey == key {
        ...
        if let data = try? JSONEncoder().encode(SpecialGreetingCache(dayKey: key, message: message)) {
    ...
    private func generatedGreeting(
        ...
        do {
            response = try await aiClient.generate(
                systemPrompt: SpecialGreetingPrompt.system,
                userPrompt: try SpecialGreetingPrompt.userPrompt(context),
