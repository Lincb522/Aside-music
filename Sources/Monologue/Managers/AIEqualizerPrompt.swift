...
enum AIEqualizerPrompt {
    static let version = "mono-audio-agent-v26"
    ...
    private static func makeSystemPrompt(bandDirective: String, gainSchema: String) -> String {
        ...
        Spatial values must follow requested tuningProfile. For standard, preserve the original image and normally keep surroundLevel within 0...0.14, reverbLevel within 0...0.07, and stereoWidth within 1...1.12. For monoSpatialEnhancement, the result must be immediately and unmistakably distinguishable from standard: use device-appropriate surround energy, audible controlled ambience, and a clearly wider stage. Headphone-like outputs should normally target surroundLevel 0.28...0.44, reverbLevel 0.24...0.44, and stereoWidth 1.22...1.40; car and AirPlay outputs should normally use surroundLevel 0.18...0.32 and reverbLevel 0.18...0.36. Built-in speakers rely mostly on ambience for spatial impression, so favor the reverb dimension: normally target surroundLevel 0.18...0.32, reverbLevel 0.24...0.40, and stereoWidth 1.14...1.26. Reduce reverb toward the lower bound when the mix is already ambience-dense, and reduce width or surround when phaseCorrelation is negative or monoCompatibility is weak. Never exceed surroundLevel 0.44, reverbLevel 0.44, or stereoWidth 1.40.
        ...
        effects is a separate compatibility stage. Always return false for loudnessNormalizationEnabled, compressorEnabled, subboostEnabled, bs2bEnabled, crossfeedEnabled, haasEnabled, virtualBassEnabled, exciterEnabled, and softclipEnabled. Mono uses its realtime EQ, professional dynamics, measured preamp, and final limiter instead of FFmpeg enhancement filters that add delay, phase shifts, synthesized harmonics, or unstable tails. Keep the final limiter enabled unless the entire plan is neutral, with a ceiling between -3 and -0.2 dBFS.
