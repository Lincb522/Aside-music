...
final class MonoNextSuiteManager: ObservableObject {
    ...
    private func recoveryHeartbeat() {
        ...
        // PlaybackTimePublisher is deliberately throttled in the background.
        // Read the renderer's real PCM counter first so a foreground/cold-start
        // heartbeat can never mistake audio that already advanced for a stall.
        ...
        // A load timestamp belongs to exactly one playback generation. If a
        // newer request replaced it, start timing that request from zero rather
        // than letting the old timeout rebuild the new pipeline immediately.
        if player.isLoading,
