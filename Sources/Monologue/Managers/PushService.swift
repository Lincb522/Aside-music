...
class PushService: NSObject, ObservableObject {
    ...
    func registerToken(_ token: String) {
        ...
        AppLogger.info("[Push] Device Token \(isNewToken ? String(localized: "已更新") : String(localized: "定期上报")) (\(env)): \(token.prefix(16))...")
    ...
    private func requestNotificationPermission() async {
        do {
            ...
            AppLogger.info("[Push] 通知权限: \(granted ? "已授权" : String(localized: "未授权"))")
        } catch {
    ...
    private func sendAPNs(authToken: String, deviceToken: String,
        ...
        if let httpResponse = response as? HTTPURLResponse {
            ...
            if httpResponse.statusCode == 401 {
                ...
                guard let json = try JSONSerialization.jsonObject(with: retryData) as? [String: Any],
                    ...
                    let msg = (try? JSONSerialization.jsonObject(with: retryData) as? [String: Any])?["message"] as? String ?? String(localized: "未知错误")
        ...
        guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
              let success = json["success"] as? Bool, success else {
            let msg = (try? JSONSerialization.jsonObject(with: data) as? [String: Any])?["message"] as? String ?? String(localized: "未知错误")
            throw PushError.sendFailed(msg)
        }
    }
    ...
    func sendDownloadCompleteNotification(songName: String) {
        send(
            ...
            body: String(localized: "\(songName) 已保存到本地")
        )
    }
    ...
    func sendDownloadFailedNotification(songName: String) {
        send(
            ...
            title: String(localized: "下载失败"),
            body: String(localized: "\(songName) 下载出错，请稍后重试")
        )
    }
    ...
    enum PushError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .sendFailed(let msg): return String(localized: "推送失败: \(msg)")
