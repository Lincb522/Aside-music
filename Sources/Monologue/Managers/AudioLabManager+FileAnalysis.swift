...
extension AudioLabManager {
    ...
    func convertQualityAssessment(_ sdkResult: AudioAnalyzer.FullAnalysisResult) -> QualityInfo {
        ...
        if dynamic.drValue < 4 {
            dynamicScore -= 50
            issues.append(String(localized: "动态范围极窄（DR\(dynamic.drValue)），严重过度压缩"))
        } else if dynamic.drValue < 6 {
            dynamicScore -= 40
            issues.append(String(localized: "动态范围过窄（DR\(dynamic.drValue)）"))
        } else if dynamic.drValue < 8 {
            dynamicScore -= 25
            issues.append(String(localized: "动态范围偏窄（DR\(dynamic.drValue)）"))
        ...
        if clipping.hasSevereClipping {
            dynamicScore -= 30
            issues.append(String(localized: "存在严重削波"))
        } else if clipping.clippingPercentage > 0.001 {
            dynamicScore -= 15
            issues.append(String(localized: "存在轻微削波"))
        }
        
        // 响度评估
        if loudness.integratedLUFS > -8 {
            if loudness.integratedLUFS > -10 {
                issues.append(String(localized: "响度过高（\(String(format: "%.1f", loudness.integratedLUFS)) LUFS），可能存在过度限制"))
            } else if loudness.integratedLUFS < -18 {
                issues.append(String(localized: "响度偏低（\(String(format: "%.1f", loudness.integratedLUFS)) LUFS）"))
            }
        }
        ...
        if freq.lowEnergyRatio > 0.55 {
            frequencyScore -= 25
            issues.append(String(localized: "低频严重过重"))
        } else if freq.lowEnergyRatio > 0.45 {
            frequencyScore -= 15
            issues.append(String(localized: "低频偏重"))
        } else if freq.lowEnergyRatio < 0.1 {
            frequencyScore -= 20
            issues.append(String(localized: "低频严重不足"))
        } else if freq.lowEnergyRatio < 0.15 {
            frequencyScore -= 10
            issues.append(String(localized: "低频偏弱"))
        }
        
        if freq.highEnergyRatio > 0.45 {
            frequencyScore -= 20
            issues.append(String(localized: "高频过亮，可能刺耳"))
        } else if freq.highEnergyRatio > 0.38 {
            frequencyScore -= 10
            issues.append(String(localized: "高频偏亮"))
        }
        
        if freq.midEnergyRatio < 0.2 {
            frequencyScore -= 15
            issues.append(String(localized: "中频凹陷，人声可能不够突出"))
        }
        
        // 频谱质心异常检测
        if freq.spectralCentroid > 5000 {
            frequencyScore -= 10
            issues.append(String(localized: "频谱质心偏高，整体偏亮"))
        } else if freq.spectralCentroid < 800 {
            frequencyScore -= 10
            issues.append(String(localized: "频谱质心偏低，整体偏暗"))
        }
        ...
        if let phase = sdkResult.phase {
            if phase.stereoWidth < 0.15 {
                frequencyScore -= 10
                issues.append(String(localized: "立体声宽度极窄，接近单声道"))
            } else if phase.correlation < -0.3 {
                frequencyScore -= 10
                issues.append(String(localized: "立体声相位异常，可能存在反相问题"))
            }
        }
        
        let overallScore = max(0, (dynamicScore + frequencyScore) / 2)
        let grade: String
        if overallScore >= 90 { grade = String(localized: "优秀") }
        else if overallScore >= 80 { grade = String(localized: "良好") }
        else if overallScore >= 65 { grade = String(localized: "一般") }
        else if overallScore >= 50 { grade = String(localized: "较差") }
        else { grade = String(localized: "很差") }
        
        if issues.isEmpty {
            issues.append(String(localized: "音频质量良好，无明显问题"))
        }
