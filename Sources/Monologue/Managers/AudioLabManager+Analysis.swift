...
extension AudioLabManager {
    ...
    /// 分析当前播放的歌曲；启用智能音效时，分析完成后自动应用到当前歌曲
    func analyzeCurrentSong() async {
        ...
        if let cached = analysisCache[song.id] {
            updateAnalysisState(cached, for: song)
            AppLogger.info("使用内存缓存的分析结果并自动应用: \(song.name)")
            return
        }
        ...
        if let persisted = loadPersistedAnalysis(songId: song.id) {
            ...
            updateAnalysisState(persisted, for: song)
            AppLogger.info("使用持久化缓存的分析结果并自动应用: \(song.name)")
            return
        }
        ...
        do {
            ...
            // 持久化到磁盘（后台执行，文件分析结果更有价值）
            if analysisMode == .file {
                let songId = song.id
                let result = analysis
                Task.detached(priority: .background) { [weak self] in
                    await self?.persistAnalysis(result, songId: songId)
                }
            }
            ...
            updateAnalysisState(analysis, for: song)
    ...
    /// 优先返回当前结果、内存缓存，再回退到持久化缓存。
    /// 该方法可供 Widget 等轻量场景复用，避免再次触发完整文件分析。
    func cachedAnalysis(for songId: Int) -> AudioAnalysisResult? {
        if lastAnalyzedSongId == songId, let currentAnalysis {
            return currentAnalysis
        }

        if let cached = analysisCache[songId] {
            return cached
        }

        if let persisted = loadPersistedAnalysis(songId: songId) {
            analysisCache[songId] = persisted
            return persisted
        }
    ...
    private func updateAnalysisState(_ analysis: AudioAnalysisResult, for song: Song) {
        guard PlayerManager.shared.currentSong?.id == song.id else {
            AppLogger.info("歌曲已切换，跳过回写旧分析结果: \(song.name)")
            return
        }
        ...
        guard isSmartEffectsEnabled else { return }
    ...
    func fetchQQSongUrlAsync(song: Song) async throws -> String {
        guard let mid = song.qqMid, !mid.isEmpty else {
            AppLogger.error("QCM歌曲缺少 mid: \(song.name)")
    ...
    /// 下载文件到临时目录（网络 I/O + 磁盘写入在后台执行）
    func downloadToTemp(url: URL, songId: Int) async throws -> URL {
        let id = songId
        return try await Task.detached(priority: .userInitiated) {
            let tempDir = FileManager.default.temporaryDirectory
            let tempFileURL = tempDir.appendingPathComponent("analysis_\(id)_\(UUID().uuidString).tmp")
