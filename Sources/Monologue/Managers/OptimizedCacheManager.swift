...
final class OptimizedCacheManager: ObservableObject {
    ...
    func quickPreload() async {
        ...
        let snapshot = await Task.detached(priority: .utility) {
            ...
            let hasDailySongs = warmedData["daily_songs"]
                .flatMap { try? decoder.decode([Song].self, from: $0) } != nil
            let hasRecommendedPlaylists = warmedData["recommend_playlists"]
                .flatMap { try? decoder.decode([Playlist].self, from: $0) } != nil
        ...
        for entry in snapshot.warmedEntries {
            ...
            memoryCache.setObject(entry.data as NSData, forKey: cacheKey, cost: entry.data.count)
        }
    ...
    func getCacheSize() -> String {
        ...
        return String(localized: "数据库: \(dbSize), 文件: \(diskSize)")
    }
...
struct CacheStatistics {
    ...
    var totalSize: String {
        return String(localized: "数据库: \(databaseSize), 文件: \(diskCacheSize)")
    }
}
