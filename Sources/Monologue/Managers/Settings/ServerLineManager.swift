...
final class ServerLineManager: ObservableObject, @unchecked Sendable {
    ...
    private func probe(line: ServerLine) async -> ProbeResult {
        ...
        let (ncm, qcm, qishui) = await (ncmProbe, qcmProbe, qishuiProbe)

        let probes = [ncm, qcm, qishui]
    ...
    private func probeEndpoint(base: String, collectActiveConnections: Bool) async -> ProbeResult {
