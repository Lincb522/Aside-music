// ServerLineManager.swift
// 多线路管理：主线路（宁波直连）+ 备用线路（云南中转）
//
...
final class ServerLineManager: ObservableObject, @unchecked Sendable {
    ...
    private func handlePeriodicTick() {
        ...
        #endif
        guard preference == .auto else { return }
        kickRefresh(trigger: .periodic)
    }
    ...
    private func performRefresh(trigger: RefreshTrigger) async -> Bool {
        ...
        async let backupProbe = probe(line: .backup)
        let (primary, backup) = await (primaryProbe, backupProbe)

        await MainActor.run {
            self.probeResults[.primary] = primary
            self.probeResults[.backup] = backup
        }
        ...
        AppLogger.info("[ServerLine] 探测(\(trigger.rawValue)) 主=\(latencyText(primary)) 备=\(latencyText(backup)) 当前=\(activeLineRawValue())")

        guard preference == .auto else { return false }

        let decision = decideAutoLine(primary: primary, backup: backup)
        ...
        case .backup:
            reason = primary.isAlive
                ? "主线路繁忙(\(latencyText(primary)))，分流到备用线路(\(latencyText(backup)))"
                : "主线路不可用，切换到备用线路"
        case .primary:
    ...
    /// 自动模式决策
    private func decideAutoLine(primary: ProbeResult, backup: ProbeResult) -> ServerLine {
        ...
        case .primary:
            ...
            if primaryLatency > Tuning.busyLatencyThreshold,
               let backupLatency = backup.latency,
               backupLatency < primaryLatency * Tuning.busyAdvantageRatio {
                return .backup
            }
            return .primary
    ...
    /// 备用线路是回源中转，5xx 意味着中转链路或主服务器故障。
    private func probe(line: ServerLine) async -> ProbeResult {
        let base = SecureConfig.apiBaseURL(for: line)
        ...
        do {
            ...
            return ProbeResult(
                latency: elapsed,
                activeConnections: Self.parseActiveConnections(from: data),
                date: Date()
            )
        } catch {
            return .failure()
